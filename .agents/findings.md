...
353. **Passing `-A` and optional `-L<log>` at Rust idalib init on Windows improves headless reproducibility and post-failure introspection:** routing init argv through `database::init()` with `-A` (auto mode) and optional log-file emission (via `IDAX_RUST_IDA_LOG`) enables collecting native IDA diagnostics in CI when open-time failures occur before wrapper-level error propagation.
