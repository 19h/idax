...
350. **Windows headless Rust example stability improves when user plugin discovery is disabled by default in shim sessions:** even after link/CRT fixes, Rust examples can exit code 1 in Windows CI with minimal diagnostics. Defaulting `idax_database_init` to `RuntimeOptions{ plugin_policy.disable_user_plugins=true }` on Windows (override via `IDAX_ENABLE_USER_PLUGINS=1`) and running CI with an isolated empty `IDAUSR` reduces plugin-induced startup/runtime instability.
