...
- [~] P20.5 Re-run `Bindings CI` matrix and close residual runtime/linking regressions (current focus: Windows Rust downstream static-link propagation for `idax_rust` and Windows headless flake gating/isolation for `binary_forensics` + `class_reconstructor`).
