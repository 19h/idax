...
### 13. Symless Microcode Operand Root Selection (Phase 47)

- **13.1. Capability Audit**
  - 13.1.1. **Action:** Compare upstream microcode operand enumeration, nested instruction numbering, destination eligibility, and injection timing with the owned graph and C++/Rust adaptation entry models.
  - 13.1.2. **Status:** Complete (P47.1; F441; KB 35.97; decision 19.50). The owned graph lacks the copied `minsn_t::modifies_d()` result required to distinguish writable destinations from store-address operands.
- **13.2. Cross-Binding Metadata Closure**
  - 13.2.1. **Action:** Add `modifies_destination` to the owned instruction snapshot and mirror it through Node and Rust with exact generated-ABI synchronization and initialized-host evidence.
  - 13.2.2. **Status:** Complete (P47.2; C++/Node/Rust structural and initialized-host copy evidence passes).
- **13.3. Bounded Port**
  - 13.3.1. **Boundary:** Enumerate only register and stack operands in depth-first execution order; inject selected sources immediately before and selected writable destinations immediately after the exact root-function instruction; retain report/apply separation and existing depth/cycle bounds. Do not approximate current pseudocode selection or resolve runtime object-dependent dispatch.
  - 13.3.2. **Status:** Complete (P47.3-P47.4; F443-F445; KB 35.99-35.101). Full cross-language validation, generated-binding identity, real IDA report/apply/fresh-process reopen evidence, documentation, and provenance checks pass; staged review and push are the remaining release operations, with no blocker.
