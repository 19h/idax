...
- 1.3.4. **Remaining focus:** rerun `Bindings CI` with corrected Windows runtime mitigations (rollback unsupported shim plugin-policy init path; retain isolated empty `IDAUSR`) alongside existing synthetic-argv + tolerant-wait behavior, then verify Rust Windows example execution succeeds.
