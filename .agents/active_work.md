...
- **10.1. P58.4 validation and closure**
  - 10.1.1. **Action:** Complete exact-file staged/redacted review, push the locally validated candidate, require green bindings/integration/validation workflows across their configured hosts, promote the same rewritten-history commit to `master`, and remove this section in the closure update.
  - 10.1.2. **Current evidence:** Rewritten owned refs pass zero-hit reachable-history scans; the fresh pinned-SDK build passes native/export/descriptor gates; and real local IDA 9.4 load, analysis, and assembly output pass after F526-F529.
  - 10.1.3. **Status:** Active; no local blocker. Replacement CI and lease-protected `master` promotion remain pending.
