...
- 1.3.3. **Latest evidence:** run `22428747919` kept all Node rows green (Windows still runtime-gated) and Rust Linux/macOS green; Rust Windows still failed with `LNK2038` RuntimeLibrary mismatch, and logs showed dual `idax-sys` output roots where workflow `RUSTFLAGS` pinned linkage to an older out dir.
- 1.3.4. **Remaining focus:** rerun `Bindings CI` after removing Windows workflow `RUSTFLAGS` merged-shim injection so current crate-emitted link metadata selects the active `idax-sys` output, then reassess whether runtime-library mismatch remains.
