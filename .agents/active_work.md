...
### 13. Symless RTTI-Adjusted Vtable Propagation (Phase 46)

- **13.1. Capability Audit**
  - 13.1.1. **Action:** Compare upstream direct/RTTI-label/data-alias vtable-load reachability and virtual-method entrypoint seeding with the current owned xref/data/function/microcode surfaces.
  - 13.1.2. **Status:** Complete (P46.1; F438; KB 35.94; decision 19.49). No public API or binding gap is required.
- **13.2. Bounded Port**
  - 13.2.1. **Scope:** For each scanned function-array candidate, inspect direct table references and, only when no direct confirmed load exists, references to `table - 2 * pointer_width`; recursively cross exact pointer-valued data aliases with a visited set, then accept only functions whose owned graph proves a pointer-width store of the exact final table address into argument zero. Seed accepted non-import table members at argument zero and merge their bounded recovered fields into the class layout.
  - 13.2.2. **Exclusions:** Do not infer a runtime object value, resolve a structure-dependent `IndirectCall`, rank inheritance heuristically, accept unconfirmed xrefs, follow non-pointer data, or model microcode-widget selection.
  - 13.2.3. **Status:** P46.2-P46.3 complete (F439; KB 35.95). Pure/live RTTI evidence and zero-addition reopen pass. Complete validation also passes across C++ 26/26, Node 238/82, Rust 139/0/17/99, generated-binding identity, the original direct-table negative control, and tracked-fixture immutability (F440; KB 35.96). P46.4 remains active only for exact staged review, commit, push, and closure synchronization.
