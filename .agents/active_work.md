...
- **6.2. Runtime Validation Sweep**
  ...
  - 6.2.3. **Blocker (Node runtime):** `bindings/node/build/Release/idax_native.node` fails to load because `@rpath/libidalib.dylib` is unresolved (probed path `<userhome>/hexrays/ida/bin/arm64_mac_clang_opt/ida.app/Contents/MacOS/libidalib.dylib` missing on this host).
