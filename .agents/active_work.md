...
- **8.1. Runtime Evidence Follow-Up**
  ...
  - 8.1.3. **Diagnostic Outcome:** `init_library` startup failures are reproducible when `IDADIR` points to SDK source roots (for example `<userhome>/dev/ida-sdk/src`) instead of a full IDA runtime root.
