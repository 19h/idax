...
### 11. Symless Forward Local-Type Replacement (Phase 42)

- **11.1. Capability Audit**
  - 11.1.1. **Action:** Compare upstream forward-declaration recognition/replacement with local named-type storage/deletion contracts and current `TypeInfo` opacity across C++, Node, and Rust.
  - 11.1.2. **Status:** Complete (P42.1; F422; KB 35.78; decision 19.45).
- **11.2. Boundary**
  - 11.2.1. **Scope:** Classify only explicit local structure/union forward declarations; replace only an exact same-name forward with a validated complete same-kind UDT; preserve complete definitions, unrelated names, and report/apply separation. Keep member-TID xrefs, multi-stroff paths, indirect calls, and widget selection separate.
  - 11.2.2. **Status:** P42.2-P42.3 complete (F423-F424; KB 35.79-35.80). C++/Node/Rust classification/replacement and both Symless adaptations are implemented; P42.4 complete-matrix validation, final documentation synchronization, staged review, and push are in progress. No blocker identified.
