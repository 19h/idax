...
- **16.232. Phase 65 Opaque Register-Value Tracking Start**
  ...
  - 16.232.22. Replacement Integrations run 29545255109 passes 3/3 and automatic audit 29545467762 passes. Earlier Bindings run 29544788057 completed after the correction push, and automatic audit 29545265951 reports only two sanitized non-runner POSIX-home categories at entries 89/100. Because prior replay evidence covers a different archive, a fresh one-shot Decision 19.72.12 diagnostic now encrypts only currently non-allowed deduplicated prefixes to a new public key while the private key remains uncommitted (F583; KB 35.236). Local round trip, exact `[skip ci]` diagnostic push, replay/decryption, immediate removal, evidence-backed correction, remaining source/audit evidence, tag cleanup, and closure remain active under P65.4.
