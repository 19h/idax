...
- **17.1. SDK Header Interface Export Repair**
  - 17.1.1. Diagnosed that the installed `idax::idax` export retained a public reference to the build-local, unexported `idasdk_headers` target and that resolving its SDK dependency must retain binary-compatible MSVC runtime selection.
  - 17.1.2. Replaced that implementation detail with `idax::sdk_headers`; the installed config now normalizes SDK/check-out roots, preserves an explicit caller CRT or applies IDAX's configured CRT, declares exact IDA SDK 9.4, and reconstructs the compile-only interface before importing `idax::idax`.
  - 17.1.3. Reconfigured, built, and installed IDAX; fresh SDK-root and checkout-root consumers using environment or CMake `IDASDK` build the SDK-bearing `ida/ui.hpp` probe, and simulated MSVC dependency loads preserve an explicit runtime while supplying the archive runtime when absent (F632; KB 2.3.1).
