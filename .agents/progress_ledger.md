...
### 17. Installed CMake Package Maintenance

- **17.1. SDK Header Interface Export Repair**
  - 17.1.1. Diagnosed that the installed `idax::idax` export retained a public reference to the build-local, unexported `idasdk_headers` target.
  - 17.1.2. Replaced that implementation detail with `idax::sdk_headers`; the installed config now declares the exact IDA SDK 9.4 dependency and reconstructs the same compile-only interface before importing `idax::idax`.
  - 17.1.3. Reconfigured, built, and installed IDAX successfully, then configured and built a clean downstream static-library consumer whose only package request is `find_package(idax REQUIRED)` and whose source includes the SDK-bearing public header `ida/ui.hpp` (F632; KB 2.3.1).
  - 17.1.4. Addressed downstream review by deriving `idasdk_DIR` from documented CMake/environment `IDASDK` only when no explicit package directory exists. Fresh env-only and `-DIDASDK`-only consumers both configure and build the SDK-bearing `ida/ui.hpp` probe without adding the SDK CMake directory to `CMAKE_PREFIX_PATH` (F633; KB 2.3.1).
