...
- **16.238. Phase 71 Opaque IDC Value and Script Execution Start**
  ...
  - 16.238.13. The isolated Node correction rebuilds under the local Apple Clang toolchain and passes all 277 structural checks, all 96 initialized-host checks, and strict TypeScript compilation. The auxiliary local GCC installation fails inside `cc1plus` before source parsing because its own ISL dynamic-library dependency is unavailable, so hosted GCC and Apple Clang replacement rows remain the authoritative A71.13 falsification probes. Exact staging/red/push, replacement matrices/audits, and closure remain active under P71.2/P71.4 with no blocker.
