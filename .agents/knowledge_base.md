...
### 35.231. Distinguish Hosted Linuxbrew Service State from User Homes [F578]
...
- Add only the exact Linuxbrew service identity to `ALLOWED_POSIX_HOMES`, assembled from byte fragments so repository sources contain no full home path. Keep arbitrary Linux/macOS users and Windows users outside the explicit hosted-runner identities fail-closed.
- Assumption A65.14 (**FALSIFIED by replay 29544303227**): the two Bindings findings are solely the official Linuxbrew prefix. The byte-identical replay remains red at entries 73 and 84 after that one addition. Dependent result is invalid; no further allowlist expansion is permitted without exact private classification. Membership checks remain `O(H)` for `H = 3` POSIX allowlist entries.
...
- Assumption A65.15: the replay runner provides OpenSSL with OAEP/SHA-256 support and public annotations preserve the ciphertext exactly. Falsify by locally round-tripping a fixture with the ephemeral key, then decrypting every replay annotation and requiring that no plaintext appears in repository bytes or public job text. Dependent result: exact private classification of the two prefixes. **Bounded risk [medium]:** the diagnostic public key and code are temporary attack surface and must be deleted in the immediate corrective commit. Encryption is `O(F)` OpenSSL invocations and 256 B ciphertext per unique finding for `F` findings.
