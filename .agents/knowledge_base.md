...
- **For Windows Rust idalib runs, include init argv `-A` and optional `-L<log>` support in the Rust init wrapper:** this increases headless determinism and captures native IDA diagnostics when failures happen inside `open_database` before wrapper error paths fire. [F353]
