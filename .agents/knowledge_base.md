...
- **Windows Rust headless sessions should suppress user plugins by default:** disabling user plugin discovery at shim init time (with env override) and setting an empty `IDAUSR` in CI helps avoid opaque runtime exit-code-1 failures after build/link success. [F350]
