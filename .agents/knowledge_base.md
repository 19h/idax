...
- Assumption A65.14: the two Bindings findings are solely the official Linuxbrew prefix. Falsify by replaying the byte-identical run 29542885658 after this one allowlist addition; any other home or canonical license token must keep the replay red. Dependent result: zero privacy false positives without admitting developer paths. **Bounded risk [low]:** if GitHub changes a service identity, the audit fails closed and requires new primary-source evidence. Membership checks remain `O(H)` for `H = 3` POSIX allowlist entries.
