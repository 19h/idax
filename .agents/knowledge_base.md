...
- 2.3. CMake Architecture
  - 2.3.1. `libidax.a` uses package-owned `idax::sdk_headers` INTERFACE target [F17, F632]
    ...
    - 2.3.1.3. Installed config normalizes SDK/check-out roots, preserves an explicit caller CRT or applies IDAX's configured CRT, finds IDA SDK 9.4, and recreates the compile-only interface before importing `idax::idax`
