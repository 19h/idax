...
`include/ida/idax.hpp`: shared error/core primitives and all 32 concept domains.
...
Implementation covers all 32 domains. The executable inventory currently
contains 874 top-level functions/types, while strict `.pyi` files cover class
methods, properties, constructors, overloads, enum members, and callback
signatures. `bindings/python/header_audit.json` locks the reviewed 34-header
