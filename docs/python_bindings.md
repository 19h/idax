...
`include/ida/idax.hpp`: shared error/core primitives and all 33 concept domains.
...
Implementation covers all 33 domains. The executable inventory currently
contains 877 top-level functions/types, while strict `.pyi` files cover class
methods, properties, constructors, overloads, enum members, and callback
signatures. `bindings/python/header_audit.json` locks the reviewed 35-header
