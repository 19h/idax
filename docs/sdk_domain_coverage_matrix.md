...
| `ida::address` | `partial` | `item_start`, `item_end`, `item_size`, `next_head`, `prev_head`, `find_first`, `find_next`, `items` | Address flags/navigation (`ida.hpp`, `bytes.hpp`) | Add missing code/data/unknown traversal iterators and ergonomic aliases (`next_defined`/`prev_defined`) | `smoke_test`, new address parity integration tests |
| `ida::data` | `partial` | `read_*`, `write_*`, `patch_*`, `original_*`, `define_*`, `undefine`, `read_string`, `read_value<T>`, `find_binary_pattern` | Byte IO + item definition (`bytes.hpp`, `offset.hpp`) | Add explicit patch revert API and additional define helpers (struct/data definition parity) | `data_mutation_safety_test`, new define/revert behavior tests |
| `ida::database` | `partial` | `init`, `open`, `save`, `close`, `file_to_database`, `memory_to_database`, `snapshots`, `set_snapshot_description` | Database lifecycle + loading (`idalib.hpp`, `loader.hpp`) | Add binary/non-binary load intent wrappers and metadata parity checks | `smoke_test`, new database-mode integration checks |
...
| Address flags/navigation | `ida.hpp`, `bytes.hpp` | `ida::address` | `partial` | Add remaining iterator/predicate parity helpers | Address integration tests |
| Byte read/write/patch | `bytes.hpp` | `ida::data` | `partial` | Add patch revert + mutation semantics hardening | Data mutation safety tests |
| Data definition/undefinition | `bytes.hpp`, `offset.hpp` | `ida::data` | `partial` | Add missing define helpers (incl. struct definition parity) | Define helper behavior tests |
| Database lifecycle/open/save/close | `idalib.hpp` | `ida::database` | `partial` | Add load intent wrappers and metadata parity checks | Database lifecycle tests |
| File/memory ingestion + snapshots | `loader.hpp`, `loader API` | `ida::database` | `partial` | Validate open/load mode and snapshot behavior parity | Snapshot + transfer tests |
