...
| `ida::segment` | `covered` | `create`, `remove`, `at`, `by_name`, `set_type`, `set_permissions`, `set_comment`, `comment`, `resize`, `move`, `first`, `last`, `next`, `prev`, `all`, `Segment::type` | Segment CRUD/properties (`segment.hpp`) | Closed in P10.3 (resize/move wrappers, segment comments, first/last/next/prev traversal) | `segment_function_edge_cases_test`, `tests/unit/api_surface_parity_test.cpp` |
...
| `ida::processor` | `covered` | `ProcessorInfo`, extended `InstructionDescriptor`/`AssemblerInfo`, `OutputContext`, `OutputInstructionResult`, `output_instruction_with_context`, `output_operand_with_context`, `Processor` optional hooks, `SwitchDescription`, `IDAX_PROCESSOR` | Processor module authoring (`idp.hpp`, `ua.hpp`) | Closed in P10.6 (output-context abstraction + advanced descriptor/assembler parity checks) | `loader_processor_scenario_test`, `tests/unit/api_surface_parity_test.cpp`, `docs/quickstart/processor.md` |
...
| Processor authoring | `idp.hpp`, `ua.hpp` | `ida::processor` | `covered` | Closed in P10.6 (OutputContext + extended descriptor/assembler surfaces) | `loader_processor_scenario_test`, compile-only parity checks |
