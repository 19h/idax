...
| `include/ida/instruction.hpp` | Decode/create, operand access + structured operand metadata (`byte_width`, `register_name`, `register_class`), representation controls, xref conveniences |
...
| `include/ida/decompiler.hpp` | Decompile (including structured failure details), pseudocode/microcode extraction, maturity subscriptions, cache-dirty helpers, microcode-filter registration, typed microcode instruction builder/emitter (`MicrocodeOpcode`/`MicrocodeOperand`/`MicrocodeInstruction`, `emit_instruction`/`emit_instructions`, `MicrocodeInsertPolicy` placement controls, additive packed bitwise/shift/add/sub/mul opcode coverage, richer typed operand/mop kinds including local-variable/register-pair/global-address/stack-variable/helper-reference/block-reference/nested-instruction), policy-aware low-level emit helpers (`emit_noop_with_policy`, `emit_move_register_with_policy`, `emit_load_memory_register_with_policy`, `emit_store_memory_register_with_policy`) plus optional UDT operand marking for store-operand and move/load/store low-level emission, temporary-register/local-variable context helpers (`allocate_temporary_register`, `local_variable_count`, `block_instruction_count`, tracked-last-emitted remove/query), typed helper-call builders (integer + float immediates + byte-array/vector/type-declaration views, declaration-driven vector element typing for advanced non-scalar/UDT layouts, local-variable/register-pair/global-address/stack-variable/helper-reference argument forms, declaration-driven typed register and immediate arguments plus typed register returns with size validation and declaration-width inference, wider-return byte-array fallback, optional argument name/flags metadata, operand writeback variants for compare/mask flows) + call options/location hints (insert-policy hint, optional auto stack-argument location assignment with start/alignment controls, register/pair/offset/register-relative/stack/static/scattered, scalar callinfo fields with auto `solid_argument_count` inference, function-role/return-location/return-type hints, and advanced callinfo list shaping for return/spoiled/passthrough/dead registers plus visible-memory ranges), variable rename/retype, ctree visitor, comment/orphan workflows, address mapping |
