...
| `include/ida/decompiler.hpp` | Decompile (including structured failure details), pseudocode/microcode extraction, maturity subscriptions, cache-dirty helpers, microcode-filter registration, typed microcode instruction builder/emitter (`MicrocodeOpcode`/`MicrocodeOperand`/`MicrocodeInstruction`, `emit_instruction`/`emit_instructions`, `MicrocodeInsertPolicy` placement controls, including additive packed bitwise/shift/add/sub/mul opcode coverage), temporary-register allocation (`allocate_temporary_register`), typed helper-call builders (integer + float immediates + byte-array/vector/type-declaration views, declaration-driven typed register arguments/returns with size validation plus wider-return byte-array fallback, optional argument name/flags metadata) + call options/location hints (insert-policy hint, optional auto stack-argument location assignment with start/alignment controls, register/pair/offset/register-relative/stack/static/scattered, scalar callinfo fields with auto `solid_argument_count` inference, function-role hint, return-location hint, return-type declaration hint), variable rename/retype, ctree visitor, comment/orphan workflows, address mapping |
