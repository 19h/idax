...
| `include/ida/decompiler.hpp` | Decompile (including structured failure details), pseudocode/microcode extraction, maturity subscriptions, cache-dirty helpers, microcode-filter registration, typed microcode instruction builder/emitter (`MicrocodeOpcode`/`MicrocodeOperand`/`MicrocodeInstruction`, `emit_instruction`/`emit_instructions`), typed helper-call builders (integer + float immediates + byte-array/vector/type-declaration views) + call options/location hints (register/pair/offset/register-relative/stack/static/scattered, scalar callinfo fields, return-type declaration hint), variable rename/retype, ctree visitor, comment/orphan workflows, address mapping |
