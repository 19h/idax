...
| `examples/plugin/symless_structure_port_plugin.cpp` | `bindings/rust/idax/examples/symless_structure_port.rs` (`Adapted`) | `N/A (plugin host-constrained)` | Same depth-bounded direct-call argument/return reconstruction engine; report is non-mutating and apply is explicit; no full Symless parity claim |
...
| `bindings/rust/idax/examples/symless_structure_port.rs` | Pass | Host-native fixture: recovered exact +4/4 B write, +8/8 B read, +18/2 B read, +24/1 B read; explicit apply added 4/4 members and changed argument 0; reopen was idempotent |
