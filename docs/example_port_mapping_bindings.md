...
| `examples/plugin/symless_structure_port_plugin.cpp` | `bindings/rust/idax/examples/symless_structure_port.rs` (`Adapted`) | `N/A (plugin host-constrained)` | Same depth-bounded direct-call argument/return engine plus declarative allocator seeds, terminal-return wrapper discovery, and fixed-size root reconstruction; report is non-mutating and apply is explicit; no full Symless parity claim |
...
| `bindings/rust/idax/examples/symless_structure_port.rs` | Pass | Argument fixture: exact intraprocedural/interprocedural fields and idempotent prototype apply. Allocator fixture: one malloc wrapper, one 32 B root, exact +4/4 B, +8/8 B, +24/1 B fields, 3/3 members, two generic prototypes, and zero-change reopen |
