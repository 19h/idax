...
| `examples/plugin/diaphora_exact_port_plugin.cpp` | `bindings/rust/idax/examples/diaphora_exact_port.rs` (`Direct`) | `N/A (plugin host-constrained)` | Byte-compatible canonical manifest; deterministic exact comparison; explicit conservative metadata apply |
...
| `bindings/rust/idax/examples/diaphora_exact_port.rs` | Pass | IDA 9.4 fixture: two byte-identical 22-record exports; 22/22 strongest-tier self matches; explicit apply changed one auto name and one absent repeatable comment; fresh reopen changed zero and retained both |
