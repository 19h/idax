...
- Severity: High
- Description: The current public wrapper still lacks decompiler write-path depth required by lifter (typed writable microcode IR construction/mutation including helper-call argument/callinfo modeling and typed mop builders, plus raw view-handle context for advanced per-view manipulations).
...
- Long-term mitigation: Add additive decompiler write-path APIs (typed microcode value/argument/call construction facade) plus carefully-scoped view-handle context bridges while preserving public opacity.
...
6. Prioritize additive decompiler write-path design for lifter-class ports: typed microcode IR value/argument/call builders (beyond current low-level context helpers) and carefully-scoped raw view-handle context bridging for advanced per-view manipulation flows.
