...
- Severity: High
- Description: The current public wrapper lacks decompiler write-path parity APIs required by lifter (`install_microcode_filter`-style hooks, writable microcode IR emission/mutation, maturity callback routing, and `FUNC_OUTLINE` + caller cache invalidation helpers).
...
- Long-term mitigation: Add additive decompiler write-path APIs (filter registration, structured microcode emit/edit facade, maturity events, and explicit outlining/cache invalidation wrappers) while preserving public opacity.
...
6. Prioritize additive decompiler write-path design for lifter-class ports: microcode-filter registration, writable microcode IR helpers, maturity callback subscriptions, and explicit `FUNC_OUTLINE` + caller cache invalidation wrappers.
