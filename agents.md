...
86. [2026-02-14] ida-qtform parity gap: no public `ida::ui::ask_form` wrapper currently exists. Impact: form-render plugins can edit/preview markup but cannot run direct `ask_form` test flows via pure idax APIs. Mitigation: add typed `ask_form` wrapper or explicitly document as intentional abstraction.
87. [2026-02-14] idalib-dump parity gap: decompiler microcode emission is not exposed in `ida::decompiler`. Impact: `--mc`/microcode-dump workflows cannot be ported without raw SDK access. Mitigation: add additive microcode retrieval/serialization facade.
...
89. [2026-02-14] idalib-dump parity gap: decompile failures via `ida::decompiler::decompile` do not currently expose rich failure location detail equivalent to raw `hexrays_failure_t::errea`. Impact: error-reporting parity for diagnostics tooling is reduced. Mitigation: extend decompiler error payloads with structured location/context fields.
...
4. Prioritize additive API design for the new external-port gaps documented in `docs/port_gap_audit_ida_qtform_idalib_dump.md` (`ui::ask_form`, decompiler microcode/failure-detail surfaces, headless plugin policy controls, Lumina facade).
