...
- Severity: High
- Description: The current public wrapper now has baseline generic typed microcode instruction emission and baseline placement/callinfo option shaping (including role + return-location + helper-call insert-policy hints plus declaration-driven typed register-argument/return widths and optional argument name/flag metadata), temporary-register allocation, additive typed packed bitwise/shift/add/sub/mul opcode emission, and the lifter probe now includes a working VMX + AVX scalar/packed subset with broad helper-fallback families (conversion/integer-arithmetic/multiply/bitwise/permute/blend/shift/compare/misc) plus mixed register/immediate/memory-source forwarding; however, it still lacks the deeper decompiler write-path depth required for full lifter parity (richer non-scalar typed writable microcode IR construction/mutation including advanced vector/UDT helper-call semantics + advanced callinfo/tmop semantics beyond current integer/float/byte-array/vector/type-declaration + register/register-pair/register-offset/register-relative/stack/static/scattered + function-role/return-location/insert-policy hint coverage, and richer typed mop builders).
