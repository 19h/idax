...
116. [2026-02-14] lifter parity incremental closure: idax now exposes decompiler maturity subscriptions (`on_maturity_changed`/`unsubscribe`/`ScopedSubscription`) and outline/cache invalidation helpers (`function::is_outlined`/`set_outlined`, `decompiler::mark_dirty`/`mark_dirty_with_callers`). Impact: stage-aware instrumentation and outlined/caller-cache flows no longer require raw SDK escapes; remaining blocker is microcode write-path/filter parity plus raw decompiler-view handles. Mitigation: keep compile-only + integration coverage and focus next API cycle on writable microcode/filter abstractions.
...
- Severity: High
- Description: The current public wrapper still lacks decompiler write-path parity APIs required by lifter (`install_microcode_filter`-style hooks, writable microcode IR emission/mutation, and raw view-handle context for advanced per-view manipulations).
...
- Long-term mitigation: Add additive decompiler write-path APIs (filter registration and structured microcode emit/edit facade) plus carefully-scoped view-handle context bridges while preserving public opacity.
...
6. Prioritize additive decompiler write-path design for lifter-class ports: microcode-filter registration, writable microcode IR helpers, and carefully-scoped raw view-handle context bridging for advanced per-view manipulation flows.
