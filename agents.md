...
- Severity: High
- Description: The current public wrapper still lacks decompiler write-path depth required by lifter (richer non-scalar typed writable microcode IR construction/mutation including advanced vector/UDT helper-call semantics + advanced callinfo/tmop semantics beyond current integer/float/byte-array/vector + register/register-pair/register-offset/register-relative/stack/static/scattered support, and typed mop builders, plus raw view-handle context for advanced per-view manipulations).
...
- Long-term mitigation: Add additive decompiler write-path APIs (richer typed microcode value/argument/callinfo construction facade beyond current integer/float/byte-array/vector helper + lightweight call-option/location-hint support) plus carefully-scoped view-handle context bridges while preserving public opacity.
