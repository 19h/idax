...
- Severity: High
- Description: The current public wrapper still lacks decompiler write-path depth required by lifter (richer non-scalar typed writable microcode IR construction/mutation including advanced vector/UDT helper-call semantics + advanced callinfo/tmop semantics beyond current integer/float/byte-array/vector/type-declaration + register/register-pair/register-offset/register-relative/stack/static/scattered support, and typed mop builders).
...
6. Prioritize additive decompiler write-path design for lifter-class ports: typed microcode IR value/argument/call builders beyond current integer/float/byte-array/vector/type-declaration + lightweight call-option/location-hint coverage (advanced vector/UDT semantics + advanced callinfo/tmop semantics).
