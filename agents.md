...
126. [2026-02-14] lifter parity incremental closure: typed helper-call arguments now include byte-array view modeling (`MicrocodeValueKind::ByteArray`) with explicit location requirements, enabling non-scalar byte-span argument typing without raw SDK callarg construction. Impact: one high-value non-scalar modeling path is now wrapper-native, but full vector/UDT argument modeling and richer typed mop/callinfo controls remain open. Mitigation: add vector/UDT typed value builders plus advanced callinfo/tmop primitives in subsequent additive slices.
127. [2026-02-14] lifter parity incremental closure: explicit helper-call argument-location hints now include register-relative placement (`ALOC_RREL` via `consume_rrel`) with validation for base-register semantics. Impact: scalar location shaping now supports additional ABI-style placement patterns without raw `argloc_t`, but full vector/UDT argument modeling and richer typed callinfo/mop controls remain open. Mitigation: continue additive non-scalar typed value + advanced callinfo/tmop builder work.
128. [2026-02-14] lifter parity incremental closure: typed helper-call arguments now include vector view modeling (`MicrocodeValueKind::Vector`) with typed element width/count/sign/floating controls plus explicit location enforcement. Impact: array-style vector argument modeling is now wrapper-native without raw SDK callarg construction, but richer UDT/vector semantics and deeper callinfo/tmop controls remain open. Mitigation: extend typed value builders toward UDT/vector semantics and advanced callinfo/tmop primitives in subsequent additive slices.
129. [2026-02-14] lifter parity incremental closure: typed helper-call arguments now include declaration-driven type views (`MicrocodeValueKind::TypeDeclarationView`) parsed via SDK `parse_decl` with explicit-location enforcement. Impact: named/complex C-type views (including many UDT-shaped declarations) can now be modeled without raw SDK callarg plumbing, but richer typed mop/callinfo semantics and raw view-handle context remain open. Mitigation: continue additive typed callinfo/mop builders and view-handle bridging driven by real port needs.
130. [2026-02-14] callinfo-shaping closure increment: `MicrocodeCallOptions` now exposes additional helper-call flags (`mark_dead_return_registers`, `mark_spoiled_lists_optimized`, `mark_synthetic_has_call`, `mark_has_format_string`) mapped to `FCI_DEAD`/`FCI_SPLOK`/`FCI_HASCALL`/`FCI_HASFMT`. Impact: lightweight call-shape control now covers more realistic decompiler call metadata without raw `mcallinfo_t` mutation, but advanced typed mop/callinfo semantics remain open. Mitigation: continue additive typed callinfo/value-location builders where concrete ports require deeper fidelity.
131. [2026-02-14] callinfo-shaping closure increment: `MicrocodeCallOptions` now includes scalar callinfo field hints (`callee_address`, `solid_argument_count`, `call_stack_pointer_delta`, `stack_arguments_top`) mapped to `mcallinfo_t` fields with explicit validation (`solid_argument_count >= 0`). Impact: helper-call shaping can now express common call metadata without raw `mcallinfo_t` mutation, while preserving additive/opaque APIs. Mitigation: keep extending typed callinfo/tmop builders for richer non-scalar and advanced location semantics.
132. [2026-02-14] Action-context host bridge closure increment: `ida::plugin::ActionContext` now includes opaque host handles (`widget_handle`, `focused_widget_handle`, `decompiler_view_handle`) and scoped callback helpers (`with_widget_host`, `with_decompiler_view_host`). Impact: advanced decompiler popup workflows can access per-view host context without exposing SDK types in public headers. Mitigation: keep the host-pointer path scoped/callback-driven in usage docs and add typed first-class decompiler-view helpers only when repeated port patterns justify them.
133. [2026-02-14] Appcall launch fallback probe: extending appcall-smoke startup attempts to include explicit fixture hold-mode args (`--wait`) does not change this host outcome — `start_process failed (return code: -1)` still occurs across all path/cwd permutations. Impact: current blocker remains debugger backend/session readiness, not fixture lifetime/race timing. Mitigation: keep dual launch-arg fallbacks for future hosts, classify this host as backend-limited, and collect pass evidence on debugger-capable runtime hosts.
134. [2026-02-14] Appcall attach fallback probe: spawning a host-native fixture and attempting `attach_process` fallback after launch failures now produces consistent `attach_process` return code `-4` across path/cwd/arg permutations on this host. Impact: this host is blocked at debugger backend/session attach readiness too (not only launch), so wrapper-side launch permutations are no longer the bottleneck. Mitigation: keep launch+attach fallback diagnostics in `ida2py_port --appcall-smoke` and gather pass evidence on a debugger-capable runtime host.
135. [2026-02-14] Callinfo-shaping closure increment: helper-call options now support a declaration-based return-type hint (`MicrocodeCallOptions::return_type_declaration`) parsed via SDK `parse_decl` and applied to `mcallinfo_t::return_type`. Impact: helper-call shaping can now describe return-type intent without raw `mcallinfo_t` mutation; invalid declarations fail early with `Validation`. Mitigation: continue additive typed callinfo/tmop builders for deeper non-scalar semantics.
136. [2026-02-14] lifter source-audit depth finding: the dominant remaining parity gap is not plugin lifecycle or popup wiring, but generic microcode instruction authoring (opcode+operand construction, callinfo/tmop depth, and deterministic insertion policy) used pervasively across AVX/VMX handlers. Impact: ad hoc helper-call-only expansion is no longer enough for full lifter migration. Mitigation: drive closure through a prioritized, source-backed additive API matrix (P0 generic typed instruction builder/emitter, then callinfo depth, then placement controls).
137. [2026-02-14] lifter parity incremental closure: `ida::decompiler::MicrocodeContext` now includes a baseline generic typed instruction emitter (`MicrocodeOpcode`, `MicrocodeOperandKind`, `MicrocodeOperand`, `MicrocodeInstruction`, `emit_instruction`, `emit_instructions`) that covers the high-value opcode family used across AVX/VMX handlers (`mov/add/xdu/ldx/stx/fadd/fsub/fmul/fdiv/i2f/f2f/nop`). Impact: full lifter migration no longer depends solely on helper-call-specific emission, but deeper typed operand/callinfo/tmop semantics are still required for full parity. Mitigation: prioritize P1/P2 additive slices (advanced callinfo depth + deterministic placement policy) after this baseline emitter closure.
138. [2026-02-14] SDK microblock insertion semantics: `mblock_t::insert_into_block(new, existing)` inserts immediately after `existing`, and passing `nullptr` inserts at block beginning. Impact: deterministic placement controls can be exposed safely without leaking raw block internals. Mitigation: provide constrained policy enums (`Tail`/`Beginning`/`BeforeTail`) in wrapper APIs instead of exposing `mblock_t*`/`minsn_t*`.
139. [2026-02-14] callinfo-shaping closure increment: `MicrocodeCallOptions` now includes additive semantic hints for helper-call role and return placement (`function_role`, `return_location`) with typed validation/mapping, avoiding raw `mcallinfo_t` mutation in public APIs. Impact: lifter-class helper-call shaping now covers more real-world callinfo semantics without SDK escapes. Mitigation: continue toward deeper per-argument metadata/tmop builders where concrete ports require more than option-hint shaping.
140. [2026-02-14] helper-call placement closure increment: helper-call insertion now supports `MicrocodeCallOptions::insert_policy` and reuses constrained `MicrocodeInsertPolicy` semantics (`Tail`/`Beginning`/`BeforeTail`) for deterministic ordering without exposing raw block internals. Impact: placement parity now covers both typed instruction emitters and helper-call paths. Mitigation: keep extending placement controls only where additional emission paths show concrete migration pressure.
141. [2026-02-14] microcode-filter runtime-stability nuance: emitting additional successful helper calls with aggressive callinfo hints (role/return-location) inside the hardening test filter path can trigger decompiler internal error `INTERR: 50765` on this fixture/host. Impact: integration tests can become flaky/non-deterministic if they mutate microcode graph too aggressively while still exercising the same decompile flow. Mitigation: keep integration coverage for these options validation-focused (compile+validation paths) and reserve heavy emission stress for dedicated host/runtime scenarios.
142. [2026-02-14] helper-call typed-return closure increment: register-return helper-call emission now accepts declaration-driven return typing in addition to integer-width defaults, with explicit size-match validation and UDT marking for wider register destinations. Impact: non-integer/wider typed return modeling is now available through wrapper-native APIs without raw `mcallinfo_t`/`mop_t` mutation. Mitigation: continue toward deeper per-argument/tmop semantics where concrete ports need more than declaration-level return typing.
143. [2026-02-14] helper-call typed-argument closure increment: register helper-call arguments now accept declaration-driven typing (`MicrocodeValue{kind=Register, type_declaration=...}`) with parse-time validation, size-match enforcement, and fallback to integer-width typing when no declaration is provided. Impact: AVX/VMX-style register argument modeling can now carry richer non-integer type intent without raw `mcallarg_t` construction. Mitigation: continue toward deeper per-argument metadata/tmop controls beyond declaration-level typing.
144. [2026-02-14] helper-call argument-metadata closure increment: typed helper-call arguments now support optional formal-name and flag metadata (`argument_name`, `argument_flags`, `MicrocodeArgumentFlag`) mapped to `mcallarg_t::name` and `mcallarg_t::flags` with unsupported-bit validation and `FAI_RETPTR -> FAI_HIDDEN` normalization. Impact: per-argument callinfo metadata can now be modeled through public APIs without raw SDK callarg mutation. Mitigation: continue toward deeper tmop/callinfo authoring where concrete ports need more than metadata + hint shaping.
145. [2026-02-14] lifter probe closure increment: `examples/plugin/lifter_port_plugin.cpp` now installs a working VMX/AVX subset through public idax microcode-filter APIs, including no-op lowering for `vzeroupper` and helper-call lowering for `vmxon/vmxoff/vmcall/vmlaunch/vmresume/vmptrld/vmptrst/vmclear/vmread/vmwrite/invept/invvpid/vmfunc`. Impact: the port probe now exercises real instruction-to-helper lowering paths instead of shell-only action wiring. Mitigation: keep expanding toward AVX-heavy handlers as deeper writable IR surfaces are added.
146. [2026-02-14] AVX temporary-register parity closure increment: `ida::decompiler::MicrocodeContext` now exposes `allocate_temporary_register(byte_width)` to mirror the common `mba->alloc_kreg` pattern used in AVX handlers for scalar/vector intermediate values. Impact: wrapper-native filters can now allocate temporaries without raw SDK escapes, reducing one high-friction write-path gap for AVX lifting flows. Mitigation: continue extending typed IR/callinfo depth for full handler parity.
147. [2026-02-14] helper-call callinfo default-shaping increment: helper-call emission now infers `solid_argument_count` from the provided argument list when call options omit it, aligning default behavior more closely with common `mcallinfo_t` builder patterns. Impact: lifter-style helper-call builders require less manual call-shape wiring while preserving explicit override control. Mitigation: keep extending deeper callinfo/tmop controls where ports need more than inferred defaults.
148. [2026-02-14] helper-call auto-stack placement shaping increment: helper-call auto-location assignment now supports additive start/alignment controls (`auto_stack_start_offset`, `auto_stack_alignment`) with validation (`non-negative` start, power-of-two positive alignment). Impact: helper-call argument placement can be made deterministic across emissions without raw `argloc_t` math. Mitigation: continue extending deeper typed callinfo/tmop semantics where ports require richer ABI modeling.
149. [2026-02-14] lifter AVX scalar subset increment: `examples/plugin/lifter_port_plugin.cpp` now lowers a baseline AVX scalar math/conversion subset (`vaddss/vsubss/vmulss/vdivss`, `vaddsd/vsubsd/vmulsd/vdivsd`, `vcvtss2sd`, `vcvtsd2ss`) through typed microcode emission (`emit_move_register` + `emit_instruction` with `FloatAdd/FloatSub/FloatMul/FloatDiv/FloatToFloat`). Impact: executable port coverage now extends beyond VMX-only lowering into concrete AVX handler patterns seen in real lifter code. Mitigation: continue expanding toward packed/vector AVX families as richer writable IR semantics are added.
150. [2026-02-14] Instruction value-object width nuance: `ida::instruction::Operand` intentionally exposes typed value fields but not rendered operand text helpers, so plugin-side width inference cannot rely on operand-text parsing from decoded operands alone. Impact: AVX scalar subset lowering currently assumes XMM-width destination copy behavior for deterministic SDK-opaque handling. Mitigation: keep scalar subset constrained/explicit and expand wrapper-level operand-width introspection APIs if broader vector-width generic lowering becomes a recurring need.
151. [2026-02-14] lifter AVX scalar subset expansion: the lifter probe now also lowers scalar min/max/sqrt/move patterns (`vminss/vmaxss/vminsd/vmaxsd`, `vsqrtss/vsqrtsd`, `vmovss/vmovsd`) using a mix of typed microcode emission and helper-call return lowering for scalar lanes. Impact: executable AVX parity now covers a broader slice of real handler families without raw SDK bridges. Mitigation: continue extending toward packed/vector families and masked forms as deeper writable IR semantics land.
152. [2026-02-14] AVX scalar move memory-destination nuance: loading destination register state (`load_operand_register(0)`) before checking for memory-destination scalar moves can introduce unnecessary failure risk for `vmovss`/`vmovsd` store forms. Impact: fragile lowering path for two-operand memory destinations. Mitigation: handle memory-destination move stores first (`store_operand_register`) and only resolve destination register state for register-target paths.
153. [2026-02-14] AVX packed subset increment: the lifter probe now lowers baseline packed math/move families (`vaddps/vsubps/vmulps/vdivps`, `vaddpd/vsubpd/vmulpd/vdivpd`, and `vmov*` packed move families) through typed microcode emission and store-aware move handling. Impact: executable AVX coverage now includes common packed two/three-operand workflows in addition to scalar lanes. Mitigation: continue toward masked/permute/blend families and richer writable IR semantics for full parity.
154. [2026-02-14] Packed-width inference nuance: while `ida::instruction::Operand` value objects do not expose rendered text, width-aware packed lowering can still use `ida::instruction::operand_text(address, index)` heuristics (`xmm`/`ymm`/`zmm` and `*word` tokens) for additive subset handling. Impact: packed AVX lowering can avoid hardcoded XMM-only widths without exposing SDK internals. Mitigation: keep heuristic parsing constrained to subset probes and promote to first-class typed width APIs if repeated port patterns demand stronger guarantees.
155. [2026-02-14] Helper-call typed-return fallback nuance: register-return helper emission now needs a non-integer fallback when destination width exceeds integer scalar widths. Impact: packed helper-return flows (`vmin*`/`vmax*`/`vsqrt*`) require a wider-type path to avoid `Unsupported` errors for 16/32/64-byte destinations. Mitigation: fallback to byte-array `tinfo_t` when no explicit return declaration is supplied and integer-width synthesis is not applicable.
156. [2026-02-14] Packed conversion subset increment: baseline packed conversion lowering (`vcvtps2pd`/`vcvtpd2ps`, `vcvtdq2ps`/`vcvtudq2ps`, `vcvtdq2pd`/`vcvtudq2pd`) can be modeled through typed `FloatToFloat`/`IntegerToFloat` emission with operand-text width heuristics. Impact: executable AVX coverage now includes common lane-wise conversion workflows in addition to arithmetic/move/extrema/sqrt families. Mitigation: keep conversion subset additive and widen only as deeper typed IR semantics prove stable in real port flows.
157. [2026-02-14] Packed helper-conversion fallback nuance: many float-int packed conversion forms (`vcvt*2dq/udq/qq/uqq`, truncating variants) do not map directly to the current typed opcode subset and require helper-call fallback paths for practical subset parity. Impact: executable probe coverage can expand to high-frequency conversion families without waiting for new typed opcodes. Mitigation: route these forms through typed helper-call emission with destination-width inference while deeper writable IR semantics remain in progress.
158. [2026-02-14] Packed addsub/horizontal nuance: `vaddsub*`/`vhadd*`/`vhsub*` require lane-level semantics not represented by current typed arithmetic opcodes (`FloatAdd`/`FloatSub` only). Impact: direct typed-instruction parity is insufficient for these families. Mitigation: use helper-call lowering for these families in the probe while keeping richer lane-aware typed IR design in the long-term roadmap.
159. [2026-02-14] Packed variadic helper-argument nuance: helper-fallback packed families (bitwise/permute/blend) can be widened safely by collecting mixed register/immediate operands from decoded instructions and forwarding them as typed helper-call arguments. Impact: executable parity breadth grows quickly without new public SDK-leaking APIs. Mitigation: keep this as probe-level additive behavior and promote repeated patterns into first-class typed IR APIs only when stabilization pressure is clear.
160. [2026-02-14] Packed bitwise/permute/blend opcode-gap nuance: current public `MicrocodeOpcode` coverage does not include direct typed operations for many packed logic/permute/blend instructions, so helper-call fallback remains the practical path for subset parity today. Impact: fast parity gains are possible, but full semantic fidelity still depends on deeper writable IR opcode/value expansion. Mitigation: maintain helper-fallback coverage in the probe and continue additive typed-opcode design for long-term closure.
161. [2026-02-14] Packed shift/rotate opcode-gap nuance: packed shift/rotate families (`vps*`, `vprol*`, `vpror*`) frequently include mixed register/immediate operand shapes that are not directly expressible in current typed opcode coverage. Impact: helper-call fallback remains the pragmatic subset path for these families. Mitigation: keep variadic helper fallback for shift/rotate in the probe and prioritize typed opcode expansion only when concrete migration fidelity requires it.
162. [2026-02-14] Variadic helper fallback robustness nuance: for broad packed fallback families, unsupported operand shapes should degrade to `NotHandled` rather than hard errors so decompiler runs stay stable while subset coverage grows. Impact: wider fallback matching (compare/misc/permute/blend/shift) can be expanded safely across mixed operand forms. Mitigation: keep variadic fallback tolerant for unsupported loads and reserve hard failures for explicit validation/runtime emission faults.
163. [2026-02-14] Variadic helper memory-operand nuance: broad packed helper-fallback families frequently include memory-source operands, so register-only extraction undercounts practical coverage. Impact: without memory-pointer argument fallback, many otherwise-portable packed forms silently miss lowering. Mitigation: when register extraction fails for a source operand, attempt effective-address register extraction and pass a typed pointer argument.
164. [2026-02-14] Packed compare destination nuance: compare families can target mask-register destinations that are not representable in current public register-load helpers. Impact: strict destination-register requirements can convert safe compare cases into unnecessary failures. Mitigation: treat unsupported compare destinations as no-op handling in the probe path (consistent with existing mask-destination tolerance goals).
165. [2026-02-14] Typed packed bitwise/shift opcode closure increment: adding `MicrocodeOpcode` members for bitwise and shift operations (`BitwiseAnd`/`BitwiseOr`/`BitwiseXor`/`ShiftLeft`/`ShiftRightLogical`/`ShiftRightArithmetic`) enables direct typed emission for a subset of packed logic/shift families before helper fallback routing. Impact: reduces helper-call reliance for high-frequency register/immediate forms while preserving additive compatibility. Mitigation: keep helper fallback for unsupported/non-direct forms (for example `andn`, rotate-specific semantics, and mixed exotic forms) until deeper typed opcode/value coverage is expanded.
166. [2026-02-15] Typed packed integer add/sub closure increment: adding `MicrocodeOpcode::Subtract` and routing `vpadd*`/`vpsub*` direct register/immediate forms through typed emission reduces packed helper-call fallback pressure for common integer vector arithmetic. Impact: practical AVX packed lowering coverage is now less helper-centric for high-frequency add/sub families while preserving additive compatibility. Mitigation: keep helper fallback for mixed/unsupported operand forms and continue additive typed opcode/value expansion as deeper parity needs appear.
167. [2026-02-15] Packed integer arithmetic operand-shape nuance: typed register/immediate emission for `vpadd*`/`vpsub*` does not cover memory-source or saturating variants by itself. Impact: without explicit fallback routing, practical packed integer forms can degrade to `NotHandled` despite safe helper-call parity being possible. Mitigation: keep typed direct emission for plain add/sub when supported, then route integer arithmetic families (including saturating forms) through variadic helper fallback when direct typed emission does not apply.
168. [2026-02-15] Packed integer multiply parity nuance: direct typed multiply emission is practical for `vpmulld`/`vpmullq` (plain multiply intent), but many other packed integer multiply variants (`vpmullw`/`vpmuludq`/`vpmaddwd`) carry lane-specific semantics not represented by the current typed opcode/value model. Impact: forcing all multiply variants into one typed path risks semantic mismatch and instability. Mitigation: keep a dual-path strategy: typed direct multiply for the safe subset, helper-call fallback for non-direct multiply variants.
169. [2026-02-15] Packed binary operand-count nuance: typed binary emission initially favored three-operand AVX forms and can miss two-operand packed encodings if destination is not treated as an implicit left operand. Impact: valid packed arithmetic/bitwise/shift/multiply forms can fall back unnecessarily to helper paths. Mitigation: in probe-side typed binary emission, treat operand 0 as both destination and left source for two-operand forms, then decode operand 1 as right source.
170. [2026-02-15] Advanced callinfo list-shaping nuance: exposing register-list and visible-memory controls in helper-call options is useful for lifter-class ports, but passthrough registers are only coherent when they are a subset of spoiled registers. Impact: unvalidated pass/spoil combinations can encode contradictory call semantics. Mitigation: when both lists are specified, validate subset semantics and return `Validation` on mismatch.
171. [2026-02-15] Declaration-driven vector element typing nuance: vector helper-call arguments can support richer non-scalar/UDT semantics by parsing `type_declaration` as element type, but element-size, count, and total-byte-width constraints must be validated together. Impact: inconsistent vector metadata can silently corrupt callarg typing. Mitigation: enforce declaration-size checks, derive missing element count from total byte width when possible, and reject mismatched shapes.
172. [2026-02-15] Rich typed mop-builder closure increment: adding `RegisterPair`/`GlobalAddress`/`StackVariable`/`HelperReference` typed operand/value kinds enables broader writable microcode IR construction without raw `mop_t` exposure. Impact: lifter-class non-scalar mutation paths can stay wrapper-native for more instruction/helper patterns. Mitigation: keep deeper tmop/nested-insn semantics as additive follow-up.
173. [2026-02-15] Block-reference typed operand increment: SDK `mop_t::make_blkref` can be surfaced safely through an additive `MicrocodeOperandKind::BlockReference` + validated `block_index` field without exposing raw microblock internals. Impact: typed instruction emission now includes one additional non-immediate control-flow-relevant operand form. Mitigation: keep nested-insn/tmop-specialized operand depth as additive follow-up.
174. [2026-02-15] Nested-instruction typed operand increment: SDK `mop_t::create_from_insn` can be surfaced safely through an additive `MicrocodeOperandKind::NestedInstruction` + `nested_instruction` payload with recursive validation/depth limiting. Impact: typed instruction emission now supports nested microinstruction composition without exposing raw `minsn_t*` in public APIs. Mitigation: keep deeper tmop-specialized operand/control semantics as additive follow-up.
175. [2026-02-15] Local-variable typed operand increment: SDK local-variable mop creation can be surfaced through additive `MicrocodeOperandKind::LocalVariable` with index/offset fields (`local_variable_index`, `local_variable_offset`) validated in wrapper space before `_make_lvar(...)` mapping. Impact: generic typed instruction emission now supports direct local-variable operands without raw `mop_t`/`lvar_t` exposure. Mitigation: keep deeper tmop-specialized/value-location semantics as additive follow-up.
176. [2026-02-15] Local-variable rewrite-path safety nuance: consuming local-variable typed operands/values in probe lowering needs context-aware availability checks to avoid brittle hardcoded-index assumptions. Impact: unguarded local-variable emission can fail on functions without decompiler locals. Mitigation: expose `MicrocodeContext::local_variable_count()` and gate local-variable rewrite usage on `count > 0` with no-op fallback.
177. [2026-02-15] Local-variable rewrite consistency nuance: duplicating local-variable self-move logic per mnemonic can drift error/fallback behavior across VMX rewrite paths. Impact: one path may silently diverge in `SdkFailure` handling or guard checks. Mitigation: centralize local-variable self-move emission in one helper and reuse it across rewrite sites (`vzeroupper`, `vmxoff`).
178. [2026-02-15] Debugger backend activation nuance: Appcall smoke reliability improves when backend discovery/loading is explicit (`available_backends` + `load_backend`) instead of assuming an already-loaded debugger. Impact: removes one hidden precondition for runtime appcall probes and gives deterministic backend diagnostics. Mitigation: expose backend discovery/selection in `ida::debugger` and auto-load appcall-capable backends in `ida2py_port` before launch attempts.
179. [2026-02-15] Appcall host-readiness nuance (current macOS host): with local `arm_mac` backend loaded, `start_process` can return `0` while `request_start` succeeds but debugger state remains `NoProcess`; attach fallback can return `-1` while `request_attach` still leaves `NoProcess`. Impact: the remaining appcall block is backend/session readiness on this host, not wrapper launch-path or API-surface coverage. Mitigation: keep this host classified as blocked and collect pass evidence on a debugger-capable host; preserve enriched diagnostics for quick triage.
180. [2026-02-15] Appcall queued-request timing nuance: `request_start`/`request_attach` can report success while debugger state still reads `NoProcess` immediately after a single queue flush. Impact: one-shot request checks can under-report host readiness and produce noisy diagnostics. Mitigation: in appcall smoke fallback paths, perform bounded multi-cycle request draining with short settle delays before classifying `NoProcess` as a backend/session block.
181. [2026-02-15] Microcode placement parity nuance: low-level decompiler emit helpers (`emit_noop`, `emit_move_register`, `emit_load_memory_register`, `emit_store_memory_register`) default to tail insertion unless placement is surfaced explicitly. Impact: deterministic rewrite ordering can drift when ports mix typed-instruction emission with low-level helper paths. Mitigation: add policy-aware low-level emit variants and route all emit paths through shared reposition logic.
182. [2026-02-15] Wide-operand low-level emit nuance: AVX/VMX-style wider register/memory flows can require UDT operand marking to avoid verifier and type-shape mismatches when using low-level emit helpers directly. Impact: ports may need raw-SDK operand mutation for some non-scalar rewrite paths. Mitigation: add optional `mark_user_defined_type` controls to low-level move/load/store emit helpers and keep validation coverage on additive overloads.
183. [2026-02-15] Store-operand UDT nuance: `store_operand_register` is often used as a high-level writeback path, but wide/non-scalar flows can still require explicit UDT marking on the source mop to preserve verifier/type-shape compatibility. Impact: without UDT control, some writeback rewrites still need raw-SDK post-build mutation. Mitigation: add additive `store_operand_register(..., mark_user_defined_type)` overload and keep validation coverage aligned with existing low-level UDT helper checks.
184. [2026-02-15] Immediate typed-argument declaration nuance: helper-call immediate arguments were integer-width-only and could not consume declaration-driven type intent. Impact: pointer/typedef-style immediate helper arguments required lossy integer fallback typing. Mitigation: allow `UnsignedImmediate`/`SignedImmediate` to consume optional `type_declaration` with parse/size validation and declaration-width inference when byte width is omitted.
185. [2026-02-15] Callinfo pass/spoil coherence nuance: `passthrough_registers` semantics are contradictory when accepted without a corresponding `spoiled_registers` superset. Impact: weak validation can encode impossible call metadata and make helper-call behavior harder to reason about. Mitigation: enforce subset semantics whenever passthrough registers are present.
...
- [2026-02-14] Add byte-array typed helper-call argument modeling (`MicrocodeValueKind::ByteArray`) with explicit-location enforcement - enables a practical non-scalar byte-span argument path while preserving SDK opacity - alternatives considered: defer all non-scalar support until full vector/UDT modeling (rejected, delays migration value), expose raw `mcallarg_t` construction in public API (rejected, opacity break) - impact: remaining lifter blocker is now narrowed from generic non-scalar coverage to richer vector/UDT and advanced typed callinfo/mop depth.
- [2026-02-14] Add register-relative explicit location hints to `MicrocodeValueLocation` (`RegisterRelative` mapped to `argloc_t::consume_rrel`) - broadens scalar helper-call location placement for register-relative ABI patterns while preserving SDK opacity - alternatives considered: keep location hints without `ALOC_RREL` support (rejected, misses practical placement cases), expose raw `rrel_t` in public API (rejected, opacity break) - impact: remaining lifter blocker is now concentrated in richer non-scalar typed values and advanced callinfo/tmop depth.
- [2026-02-14] Add vector typed helper-call argument modeling (`MicrocodeValueKind::Vector`) with typed element controls and explicit-location enforcement - enables array-style vector argument typing without raw SDK callarg construction while preserving SDK opacity - alternatives considered: defer vector modeling until full UDT abstraction (rejected, delays migration value), expose raw `mcallarg_t`/type plumbing publicly (rejected, opacity break) - impact: remaining lifter blocker is now concentrated in richer UDT/vector semantics and advanced typed callinfo/mop depth.
- [2026-02-14] Add declaration-driven typed helper-call argument modeling (`MicrocodeValueKind::TypeDeclarationView`) with SDK `parse_decl` integration and explicit-location enforcement - enables C-declaration-backed non-scalar argument views without exposing raw SDK callarg constructors - alternatives considered: defer declaration-based support until full UDT APIs (rejected, delays practical migration value), expose raw `tinfo_t`/`mcallarg_t` in public API (rejected, opacity break) - impact: remaining lifter blocker is now narrowed from generic UDT declaration coverage to richer typed mop/callinfo semantics and view-handle parity.
- [2026-02-14] Expand helper-call option shaping with additional `FCI_*` controls (`mark_dead_return_registers`, `mark_spoiled_lists_optimized`, `mark_synthetic_has_call`, `mark_has_format_string`) - improves wrapper-native call metadata tuning while preserving SDK opacity - alternatives considered: keep minimal call flags only (rejected, too restrictive for real call-shape scenarios), expose raw `mcallinfo_t` flag mutation in public API (rejected, opacity break) - impact: remaining lifter blocker is now focused more tightly on advanced typed mop/callinfo semantics and view-handle parity.
- [2026-02-14] Expand helper-call option shaping with scalar callinfo field hints (`callee_address`, `solid_argument_count`, `call_stack_pointer_delta`, `stack_arguments_top`) and validation for invalid counts - improves wrapper-native call metadata control without exposing raw `mcallinfo_t` field mutation in public APIs - alternatives considered: keep field-level shaping internal only (rejected, insufficient migration fidelity), expose raw `mcallinfo_t` mutators (rejected, opacity break) - impact: remaining lifter blocker is narrowed further to advanced typed mop/callinfo semantics and view-handle parity.
- [2026-02-14] Add action-context host bridges in `ida::plugin` (`ActionContext::{widget_handle,focused_widget_handle,decompiler_view_handle}`, `widget_host`, `with_widget_host`, `decompiler_view_host`, `with_decompiler_view_host`) - enables advanced popup/decompiler workflows to access scoped host context without leaking SDK types in public headers - alternatives considered: keep normalized context fields only (rejected, blocks high-value lifter popup flows), expose raw SDK types in callbacks (rejected, breaks opaque boundary) - impact: lifter blocker scope is narrowed from write-path depth + view-handle access to predominantly write-path depth (advanced typed microcode/callinfo/tmop semantics).
- [2026-02-14] Expand appcall-smoke startup strategy to try both fixture hold-mode and default launches (`--wait` then empty args) across candidate path/cwd permutations - de-risks process-lifetime race explanations before classifying host failures as debugger-backend limitations - alternatives considered: keep default-args-only startup (rejected, weaker diagnosis), force attach-only fallback first (rejected, requires additional long-lived fixture/process orchestration) - impact: appcall failure diagnostics are now stronger and host limitations can be classified with higher confidence.
- [2026-02-14] Add spawn+attach fallback probing to appcall smoke (`ida2py_port`) after direct debugger launch failures - distinguishes launch-path issues from debugger attach/backend readiness issues with concrete error-code evidence (`attach_process` return codes) - alternatives considered: keep launch-only fallback probes (rejected, ambiguous root-cause classification), add standalone attach-only utility binary (rejected, avoid target sprawl) - impact: open-point classification is now more surgical and reliably attributable to host debugger backend limitations.
- [2026-02-14] Expand helper-call option shaping with declaration-based return-type hints (`MicrocodeCallOptions::return_type_declaration`) parsed by `parse_decl` and applied to `mcallinfo_t::return_type` - improves wrapper-native call metadata control for helper-call return typing without exposing raw SDK callinfo mutation in public APIs - alternatives considered: keep return-type shaping implicit only via destination register overloads (rejected, insufficient fidelity), expose raw `mcallinfo_t`/`tinfo_t` mutation publicly (rejected, opacity break) - impact: helper-call shaping depth now includes return-type intent while preserving additive opaque APIs.
- [2026-02-14] Execute lifter follow-up via a source-backed gap matrix in `docs/port_gap_audit_lifter.md` with explicit closure slices (P0 generic typed instruction builder/emitter, P1 callinfo depth, P2 placement policy, P3 typed view ergonomics) - prevents drift between high-level blocker wording and concrete port requirements seen in `<userhome>/dev/lifter` - alternatives considered: keep broad blocker-only wording (rejected, weak implementation guidance), jump directly to large raw-SDK mirror surface (rejected, opacity and stability risk) - impact: decompiler write-path closure work is now ordered, auditable, and tied to concrete source evidence.
- [2026-02-14] Add baseline generic typed instruction emission to `ida::decompiler::MicrocodeContext` (`MicrocodeOpcode`/`MicrocodeOperandKind`/`MicrocodeOperand`/`MicrocodeInstruction`, `emit_instruction`, `emit_instructions`) - closes the highest-priority lifter write-path slice while preserving SDK opacity and additive compatibility - alternatives considered: continue helper-call-only expansion first (rejected, insufficient for broad AVX/VMX handlers), expose raw `minsn_t`/`mop_t` builders in public API (rejected, opacity break) - impact: lifter closure now progresses from generic opcode emission toward deeper typed operand/callinfo/tmop semantics.
- [2026-02-14] Add constrained placement-policy controls for typed microcode emission (`MicrocodeInsertPolicy`, `emit_instruction_with_policy`, `emit_instructions_with_policy`) - enables deterministic block insertion semantics for wrapper-native typed emit paths while preserving SDK opacity - alternatives considered: expose raw `mblock_t::insert_into_block`/`minsn_t*` handles (rejected, opacity break), keep tail-only insertion (rejected, insufficient for real lifter ordering flows) - impact: P2 placement-policy baseline is now closed for typed instruction emission and remaining placement work is narrowed to additional emission paths.
- [2026-02-14] Expand helper-call option shaping with semantic role and return-location hints (`MicrocodeFunctionRole`, `MicrocodeCallOptions::{function_role, return_location}`) - increases callinfo authoring fidelity for wrapper-native helper-call emission while preserving SDK opacity - alternatives considered: expose raw `funcrole_t`/`argloc_t`/`mcallinfo_t` mutators in public API (rejected, opacity break), keep only scalar callinfo hints (rejected, insufficient lifter parity progression) - impact: P1 baseline callinfo-depth closure now includes role + return-placement semantics.
- [2026-02-14] Extend helper-call emission with insertion-policy hinting (`MicrocodeCallOptions::insert_policy`) - aligns helper-call placement controls with typed-instruction placement semantics while preserving additive opaque APIs - alternatives considered: add separate helper-call-with-policy overload family (rejected, API bloat), expose raw block/anchor handles in public API (rejected, opacity break) - impact: P2 placement coverage now includes helper-call paths in addition to typed instruction emitters.
- [2026-02-14] Expand helper-call register-return typing with declaration-driven non-integer widths and size validation - improves AVX/VMX-style typed return modeling while preserving integer-default ergonomics and opaque APIs - alternatives considered: keep register-return helper calls integer-only (rejected, insufficient for wider typed returns), expose raw `mcallinfo_t`/`mop_t` return mutation in public API (rejected, opacity break) - impact: helper-call return modeling now supports declaration-driven typed widths for richer lifter-style flows.
- [2026-02-14] Expand helper-call register-argument typing with declaration-driven non-integer widths and size validation - improves AVX/VMX-style typed register-argument modeling while preserving existing integer-default ergonomics and opaque APIs - alternatives considered: keep register arguments integer-only (rejected, insufficient for richer helper signatures), require `TypeDeclarationView` + explicit location for all typed registers (rejected, less ergonomic for common register-arg paths) - impact: register helper-call arguments now support declaration-driven typed widths for richer lifter-style flows.
- [2026-02-14] Expand typed helper-call arguments with optional formal-name and flag metadata (`MicrocodeValue::{argument_name,argument_flags}`, `MicrocodeArgumentFlag`) - improves wrapper-native per-argument call metadata control while preserving SDK opacity - alternatives considered: keep metadata implicit only (rejected, insufficient callinfo fidelity), expose raw `mcallarg_t` mutation in public API (rejected, opacity break) - impact: helper-call argument metadata parity now includes public name/flag modeling with validation.
- [2026-02-14] Add VMX microcode-lifting subset to `examples/plugin/lifter_port_plugin.cpp` using public idax filter/context APIs - converts the lifter probe from shell-only workflows to concrete instruction-lowering behavior without raw SDK bridges - alternatives considered: keep probe read-only and gaps-only (rejected, weaker migration evidence), port full AVX/VMX handlers in one step (rejected, blocked by remaining deep write-path APIs) - impact: the port probe now validates real lifting paths and narrows remaining blocker scope to advanced writable IR depth.
- [2026-02-14] Add temporary-register allocation to `ida::decompiler::MicrocodeContext` (`allocate_temporary_register`) - closes a common AVX migration gap where handlers rely on `mba->alloc_kreg` for intermediate scalar/vector values - alternatives considered: keep temporary allocation raw-SDK-only in port probes (rejected, preserves escape hatches), infer temps indirectly via load helpers only (rejected, insufficient for many rewrite patterns) - impact: AVX-style filter logic can now reserve temporaries through pure idax APIs.
- [2026-02-14] Extend lifter probe subset with AVX `vzeroupper` no-op lowering - broadens executable real-port coverage beyond VMX-only flows while preserving public API opacity - alternatives considered: keep VMX-only subset until full AVX handler parity (rejected, slower real-port signal), attempt broad AVX handler port immediately (rejected, still blocked by deeper writable IR semantics) - impact: executable probe now demonstrates mixed VMX/AVX lowering through pure idax APIs.
- [2026-02-14] Add default helper-call `solid_argument_count` inference from provided argument lists - reduces manual call-shape boilerplate in lifter-style builders while preserving explicit `MicrocodeCallOptions::solid_argument_count` override semantics - alternatives considered: keep all solid-arg shaping explicit at call sites only (rejected, repetitive and error-prone), hardcode one value regardless of argument count (rejected, incorrect for variable arity) - impact: helper-call callinfo defaults now mirror common `mcallinfo_t` builder intent more closely.
- [2026-02-14] Add helper-call auto-stack placement shaping controls (`auto_stack_start_offset`, `auto_stack_alignment`) - enables deterministic auto location assignment for helper-call arguments without manual explicit-location wiring while preserving opaque/additive APIs - alternatives considered: keep auto placement as fixed internal heuristic only (rejected, limited deterministic control), require explicit location for every non-scalar argument (rejected, heavier migration boilerplate) - impact: helper-call auto placement can now be tuned per emission while keeping default behavior simple.
- [2026-02-14] Extend lifter probe with baseline AVX scalar math/conversion lowering (`vaddss/vsubss/vmulss/vdivss`, `vaddsd/vsubsd/vmulsd/vdivsd`, `vcvtss2sd`, `vcvtsd2ss`) via typed microcode emission - broadens executable real-port coverage into concrete AVX handler patterns without introducing raw SDK bridges - alternatives considered: keep VMX-only subset until broader vector API closure (rejected, weaker migration signal), jump directly to packed/vector AVX family support in one pass (rejected, higher risk and larger change surface) - impact: the executable probe now validates mixed VMX + AVX scalar lowering through pure idax APIs.
- [2026-02-14] Keep AVX scalar subset lowering explicitly XMM-oriented for now - decoded `ida::instruction::Operand` value objects do not expose rendered operand-width text directly, so broad generic YMM/ZMM width inference in plugin code would be brittle without adding new wrapper introspection APIs - alternatives considered: parse disassembly text ad hoc in the plugin (rejected, brittle and format-dependent), overgeneralize to fixed wider widths (rejected, higher correctness risk) - impact: scalar subset remains deterministic today while preserving a clear additive path for future width-introspection APIs.
- [2026-02-14] Expand lifter probe scalar AVX lowering to include min/max/sqrt/move families (`vmin*`/`vmax*`, `vsqrt*`, `vmov*`) - increases executable real-port coverage with low-risk additive logic that composes existing typed emit and helper-return APIs - alternatives considered: keep only add/sub/mul/div+convert baseline (rejected, leaves common scalar families unexercised), jump straight to packed/vector families (rejected, larger correctness surface per change) - impact: scalar AVX coverage now better matches real handler distribution while preserving incremental migration safety.
- [2026-02-14] Handle `vmovss`/`vmovsd` memory-destination forms before destination-register loading in the probe - avoids unnecessary destination-register resolution for store-only scalar moves and keeps lowering robust for two-operand memory targets - alternatives considered: keep one-path destination-register-first flow (rejected, brittle for memory destinations), skip memory-destination scalar moves entirely (rejected, leaves a common pattern unlifted) - impact: scalar move lowering now handles both register and memory destination forms more safely.
- [2026-02-14] Expand lifter probe from AVX scalar-only lowering to include baseline packed math/move families (`vaddps/vsubps/vmulps/vdivps`, `vaddpd/vsubpd/vmulpd/vdivpd`, `vmov*`) with operand-width heuristics from `ida::instruction::operand_text` - broadens executable real-port coverage with low-risk additive typed-emission paths while preserving SDK opacity - alternatives considered: jump directly to masked packed families (rejected, larger correctness surface), keep scalar-only probe until deeper IR APIs land (rejected, weaker migration pressure) - impact: executable probe now validates mixed VMX + AVX scalar/packed lowering through pure idax APIs.
- [2026-02-14] Expand helper-call register-return typing fallback for wider destinations - when no explicit return declaration is supplied and width is beyond integer scalar sizes, synthesize a byte-array return type to preserve additive helper-return modeling for packed flows - alternatives considered: keep `Unsupported` for widths >8 (rejected, blocks packed helper-return patterns), require explicit declaration at every wide call site (rejected, excessive boilerplate for subset probes) - impact: packed helper-return lowering paths (`vmin*`/`vmax*`/`vsqrt*`) can now be expressed through existing public APIs.
- [2026-02-14] Expand lifter probe packed subset with min/max/sqrt families (`vminps/vmaxps/vminpd/vmaxpd`, `vsqrtps/vsqrtpd`) - increases executable AVX parity using the existing helper-return composition path while keeping API opacity intact - alternatives considered: postpone packed min/max/sqrt until deeper writable IR APIs (rejected, slows real-port coverage), force typed-emitter-only implementation (rejected, missing direct opcode parity for min/max/sqrt) - impact: packed AVX subset now covers common arithmetic + extrema + sqrt + move workflows.
- [2026-02-14] Expand lifter probe packed subset with baseline conversion families (`vcvtps2pd`/`vcvtpd2ps`, `vcvtdq2ps`/`vcvtudq2ps`, `vcvtdq2pd`/`vcvtudq2pd`) - broadens executable AVX parity with low-risk additive typed emission (`FloatToFloat`/`IntegerToFloat`) while preserving SDK opacity - alternatives considered: defer conversions until full vector/tmop DSL (rejected, delays high-frequency real-port patterns), encode conversion family as helper-call-only paths (rejected, less direct opcode parity where typed emit exists) - impact: executable probe now covers arithmetic + extrema + sqrt + move + conversion packed workflows.
- [2026-02-14] Expand packed conversion probe coverage with helper-call fallback families (`vcvt*2dq/udq/qq/uqq`, including truncating variants) - broadens executable AVX parity for high-frequency conversion classes that are not directly representable by current typed conversion opcodes - alternatives considered: postpone until new typed opcodes exist (rejected, delays real-port parity), force inaccurate typed-opcode mappings (rejected, semantic risk) - impact: conversion coverage now spans both typed-opcode and helper-fallback paths.
- [2026-02-14] Expand packed probe coverage with addsub/horizontal families (`vaddsub*`, `vhadd*`, `vhsub*`) via helper-call lowering - covers common lane-wise math families without exposing raw SDK internals while typed lane-aware IR remains additive follow-up work - alternatives considered: skip these families until full lane-aware typed IR exists (rejected, weaker executable port pressure), approximate through plain add/sub opcodes (rejected, semantic mismatch) - impact: executable packed subset now better reflects real AVX handler diversity.
- [2026-02-14] Expand packed probe coverage with variadic helper-fallback bitwise/permute/blend families - collecting mixed register/immediate operands from decoded instruction values allows broad additive subset expansion without exposing raw SDK internals - alternatives considered: wait for new typed opcodes first (rejected, slower parity progress), add many one-off bespoke handlers per mnemonic (rejected, higher maintenance churn) - impact: executable AVX subset now covers additional high-frequency logic/permute/blend families while preserving opaque API boundaries.
- [2026-02-14] Expand packed probe coverage with variadic helper-fallback shift/rotate families (`vps*`, `vprol*`, `vpror*`) - mixed register/immediate operand forwarding closes additional high-frequency packed workflows without exposing SDK internals - alternatives considered: postpone until typed shift/rotate opcodes exist (rejected, slower practical parity), add per-mnemonic bespoke handlers only (rejected, maintenance-heavy) - impact: executable AVX subset now covers common packed shift/rotate families while preserving additive opaque APIs.
- [2026-02-14] Keep variadic helper fallback tolerant for unsupported packed operand shapes (`NotHandled` over hard error) while expanding compare/misc coverage - preserves decompiler stability as broad mnemonic families are added incrementally through mixed register/immediate forwarding - alternatives considered: strict erroring on any unsupported operand load (rejected, brittle for mixed memory/register forms), delaying broad family matching until full typed-IR opcodes exist (rejected, slower practical parity gains) - impact: wider packed fallback coverage can be added safely without destabilizing decompile passes.
- [2026-02-14] Extend variadic helper fallback to accept memory-source operands via effective-address register extraction and typed pointer arguments - broad packed families often mix register/immediate/memory sources, and register-only extraction under-represents practical parity - alternatives considered: keep register-only fallback (rejected, misses many real forms), fail hard on memory sources (rejected, unnecessary instability) - impact: helper-fallback breadth now better matches real packed instruction operand shapes.
- [2026-02-14] Treat unsupported compare mask-destination forms as no-op in the probe fallback path - aligns with existing k-register tolerance strategy and avoids brittle failures where destination is not representable via current public register-load helpers - alternatives considered: hard-fail on non-register compare destinations (rejected, destabilizing), defer compare fallback expansion entirely (rejected, slower practical parity) - impact: compare-family fallback expansion remains stable while preserving additive/opaque APIs.
- [2026-02-14] Add typed packed bitwise/shift opcode coverage in `ida::decompiler` (`MicrocodeOpcode::{BitwiseAnd,BitwiseOr,BitwiseXor,ShiftLeft,ShiftRightLogical,ShiftRightArithmetic}` + SDK mapping) and opportunistically use those opcodes in the lifter probe before helper fallback - reduces helper-call dependence for common register/immediate packed forms while preserving additive compatibility - alternatives considered: keep all packed bitwise/shift in helper fallback only (rejected, weaker typed-IR parity), add very broad opcode set in one step (rejected, higher regression risk) - impact: practical decompiler write-path depth improves and probe behavior is less helper-centric for high-frequency operations.
- [2026-02-15] Add `MicrocodeOpcode::Subtract` and use typed packed integer add/sub emission (`vpadd*`/`vpsub*`) in the lifter probe before helper fallback - improves direct typed coverage for high-frequency packed integer arithmetic without expanding public SDK exposure - alternatives considered: keep integer add/sub in helper fallback only (rejected, weaker typed-IR parity and more helper churn), add a broader integer/vector opcode surface in one pass (rejected, higher regression risk) - impact: packed integer AVX lowering is now more deterministic and less helper-dependent for direct forms.
- [2026-02-15] Keep packed integer arithmetic lowering dual-path (typed first, helper fallback second) and add saturating-family helper routing (`vpaddsb`/`vpaddsw`/`vpaddusb`/`vpaddusw`, `vpsubsb`/`vpsubsw`/`vpsubusb`/`vpsubusw`) - preserves semantic safety while preventing mixed operand forms from regressing to `NotHandled` - alternatives considered: map saturating forms onto plain typed `Add`/`Subtract` (rejected, semantic mismatch), keep only typed direct path for integer add/sub families (rejected, misses memory/saturating practical forms) - impact: packed integer AVX coverage is broader and more stable without raw SDK escapes.
- [2026-02-15] Add `MicrocodeOpcode::Multiply` and use typed packed integer multiply emission for `vpmulld`/`vpmullq` before helper fallback - improves direct typed coverage for safe packed integer multiply forms without overfitting non-direct lane semantics - alternatives considered: keep all integer multiply in helper fallback only (rejected, weaker typed-IR parity for common forms), map all multiply variants to typed multiply directly (rejected, semantic mismatch risk for `vpmullw`/`vpmuludq`/`vpmaddwd`) - impact: packed integer multiply coverage is broader while retaining semantic safety through fallback routing.
- [2026-02-15] Treat two-operand packed binary encodings as destination-implicit-left-source in typed probe emission - preserves additive typed lowering for non-VEX/legacy operand shapes without changing public API surface - alternatives considered: keep three-operand-only typed binary path (rejected, unnecessary helper fallback churn), force helper fallback for all two-operand forms (rejected, weaker typed-IR parity) - impact: typed add/sub/mul/bitwise/shift paths now apply to a wider practical AVX packed shape set.
- [2026-02-15] Expand `ida::decompiler` typed writable IR surface with richer non-scalar and callinfo/tmop semantics (declaration-driven vector element typing, typed operand/value mop builders for register-pair/global-address/stack-variable/helper-reference, and callinfo list shaping for return/spoiled/passthrough/dead registers plus visible-memory ranges) - addresses the highest-value lifter-class write-path depth requested after baseline opcode/placement closure - alternatives considered: keep option-hint-only callinfo surface (rejected, insufficient parity for complex helper-call shaping), expose raw `mop_t`/`mcallinfo_t` mutators publicly (rejected, breaks opaque boundary) - impact: advanced non-scalar helper-call/instruction construction is broader while preserving additive opaque APIs.
- [2026-02-15] Add `MicrocodeOperandKind::BlockReference` in `ida::decompiler` with validated index mapping to SDK `mop_t::make_blkref` - closes one practical typed-operand gap for microcode instruction emission without exposing raw block pointers or `mop_t` internals - alternatives considered: keep block references raw-SDK-only (rejected, unnecessary opaque-surface gap), expose raw block handles in public API (rejected, breaks opacity) - impact: typed instruction emission now supports an additional block-reference operand form while preserving additive compatibility.
- [2026-02-15] Add `MicrocodeOperandKind::NestedInstruction` in `ida::decompiler` with recursive typed payload support (`MicrocodeOperand::nested_instruction`) mapped via SDK `mop_t::create_from_insn` and depth-limited validation - closes a practical typed nested-insn authoring gap for microcode instruction emission without exposing raw `minsn_t*` in public APIs - alternatives considered: keep nested instructions raw-SDK-only (rejected, unnecessary opaque-surface gap), expose raw `minsn_t*`/builders directly (rejected, breaks opacity and ownership safety) - impact: typed instruction emission now supports additive nested instruction composition while preserving opaque boundaries.
- [2026-02-15] Add `MicrocodeOperandKind::LocalVariable` in `ida::decompiler` with validated `local_variable_index`/`local_variable_offset` mapping to SDK local-variable mop construction (`_make_lvar`) - closes a practical typed local-variable authoring gap for microcode instruction emission without exposing raw `mop_t`/`lvar_t` internals - alternatives considered: keep local-variable operands raw-SDK-only (rejected, unnecessary opaque-surface gap), expose raw local-variable SDK types in public API (rejected, breaks opacity) - impact: typed instruction emission now supports additive local-variable operands for lifter-class transformations while preserving opaque boundaries.
- [2026-02-15] Expand local-variable microcode shaping with value-side modeling + context-aware guards (`MicrocodeValueKind::LocalVariable`, `MicrocodeValue::{local_variable_index, local_variable_offset}`, `MicrocodeContext::local_variable_count`) and consume it in lifter probe rewrite paths with safe fallback - rationale: enables practical local-variable-aware rewrites without raw SDK access while avoiding invalid-index assumptions - alternatives considered: keep local-variable support instruction-only (rejected, leaves helper/value paths incomplete), hardcode local-variable indices in examples (rejected, brittle) - impact: local-variable typed emission is now available across operand/value paths and used in executable probe logic.
- [2026-02-15] Consolidate lifter local-variable rewrite behavior behind a shared helper (`try_emit_local_variable_self_move`) and apply it to multiple VMX rewrite sites (`vzeroupper`, `vmxoff`) - keeps local-variable guard/error/fallback semantics consistent across mnemonics while preserving additive public API boundaries - alternatives considered: duplicate per-mnemonic local-variable logic (rejected, drift-prone), limit local-variable rewrite usage to one mnemonic only (rejected, weaker executable parity pressure) - impact: probe-side local-variable-aware rewrites are broader and behaviorally consistent.
- [2026-02-15] Add debugger backend discovery/selection surface (`BackendInfo`, `available_backends`, `current_backend`, `load_backend`) plus queued launch/attach helpers (`request_start`, `request_attach`) to `ida::debugger` - removes hidden debugger-backend preconditions for runtime probes and gives deterministic backend diagnostics without exposing SDK internals - alternatives considered: keep backend probing/tooling logic private in examples only (rejected, weak discoverability), rely on synchronous start/attach only (rejected, misses async request path behavior) - impact: runtime probes and downstream tools can reason explicitly about backend capability/selection.
- [2026-02-15] Upgrade `ida2py_port --appcall-smoke` to backend-aware + multi-path execution (`load` appcall-capable backend, then `start` + `request_start` + `attach` + `request_attach` with state checks) - maximizes practical success chances on constrained hosts while preserving clear blocked-vs-fail classification - alternatives considered: keep launch-only fallback probes (rejected, less diagnostic depth), attempt host-specific debugger hacks (rejected, brittle/non-portable) - impact: remaining appcall blockers are now clearly attributable to host backend/session readiness.
- [2026-02-15] Add bounded queue-drain settling for appcall startup/attach request fallbacks (`run_requests` cycles + short delays + repeated state checks) - reduces false-negative `NoProcess` classifications from one-shot request polling while preserving explicit blocked diagnostics when state never transitions - alternatives considered: keep one-shot `run_requests` checks only (rejected, noisy under asynchronous hosts), add unbounded polling (rejected, can hang validation tooling) - impact: appcall smoke diagnostics are more deterministic and actionable on constrained debugger hosts.
- [2026-02-15] Add policy-aware placement controls for decompiler low-level emit helpers (`emit_noop_with_policy`, `emit_move_register_with_policy`, `emit_load_memory_register_with_policy`, `emit_store_memory_register_with_policy`) and route existing helpers through those defaults - extends deterministic insertion semantics beyond typed instruction and helper-call APIs without exposing SDK block internals - alternatives considered: keep low-level helpers tail-only (rejected, uneven placement parity), add bespoke placement args to each call site only in examples (rejected, brittle and non-discoverable) - impact: lifter-style rewrite flows can keep deterministic ordering when mixing typed and low-level emission paths.
- [2026-02-15] Add optional UDT-marking controls to low-level decompiler move/load/store emit helpers (including policy-aware overloads) - improves non-scalar/wide operand rewrite fidelity without exposing raw `mop_t` mutation in public APIs - alternatives considered: keep UDT shaping limited to typed-instruction builders only (rejected, leaves low-level helper gap), require raw SDK post-emit operand mutation in ports (rejected, weakens opaque migration path) - impact: additive low-level emit paths now support wider typed semantics needed by lifter-class rewrites.
- [2026-02-15] Add optional UDT-marking control to `store_operand_register` (`store_operand_register(..., mark_user_defined_type)`) - extends high-level writeback parity for wide/non-scalar rewrite flows without exposing raw SDK operand mutation in public APIs - alternatives considered: keep `store_operand_register` integer/default-only (rejected, leaves residual raw-SDK gap for writeback paths), route all writebacks through lower-level emit helpers only (rejected, loses ergonomic direct-write path) - impact: additive writeback APIs now cover one more high-value non-scalar parity case.
- [2026-02-15] Expand helper-call typed-argument modeling to allow declaration-driven immediate arguments (`UnsignedImmediate`/`SignedImmediate` with optional `type_declaration`) - improves typed helper signature fidelity for pointer/typedef-style immediates while preserving additive opaque APIs - alternatives considered: keep immediates integer-only (rejected, loses declaration intent), add a separate immediate-declaration value kind (rejected, unnecessary API surface growth) - impact: immediate helper arguments now support declaration parsing, size validation, and inferred width when byte width is omitted.
- [2026-02-15] Tighten callinfo list-shaping validation so `passthrough_registers` must always be a subset of `spoiled_registers` - prevents contradictory helper-call metadata when pass-through ranges are provided without a spoil superset - alternatives considered: keep conditional validation only when both lists are specified (rejected, permits inconsistent states), auto-promote pass-through ranges into spoiled list silently (rejected, obscures caller intent/errors) - impact: helper-call callinfo semantics are stricter and easier to reason about.
...
- Scope impacted: Full idax-first port of `<userhome>/dev/lifter` (AVX/VMX microcode transformations)
- Severity: High
- Description: The current public wrapper now has baseline generic typed microcode instruction emission and baseline placement/callinfo option shaping (including role + return-location + helper-call insert-policy hints plus declaration-driven typed register-argument/return widths and optional argument name/flag metadata), temporary-register allocation, local-variable context querying (`local_variable_count`), additive typed packed bitwise/shift/add/sub/mul opcode emission, richer typed operand/value mop builders (`LocalVariable`/`RegisterPair`/`GlobalAddress`/`StackVariable`/`HelperReference`/`BlockReference`/`NestedInstruction`), declaration-driven vector element typing, and advanced callinfo list shaping (return/spoiled/passthrough/dead registers + visible-memory ranges); the lifter probe includes a working VMX + AVX scalar/packed subset with broad helper-fallback families (conversion/integer-arithmetic/multiply/bitwise/permute/blend/shift/compare/misc) plus mixed register/immediate/memory-source forwarding. Remaining parity depth is now concentrated in deeper tmop semantics and other advanced decompiler write-path surfaces needed for full lifter parity.
...
- Long-term mitigation: Add additive decompiler write-path APIs (richer typed microcode value/argument/callinfo construction facade beyond current integer/float/byte-array/vector/type-declaration helper + lightweight call-option/location-hint support) while preserving public opacity.
...
- [2026-02-14 21:20] Lifter parity follow-up - Added non-scalar byte-array helper-call argument modeling (`MicrocodeValueKind::ByteArray`) with explicit-location enforcement, expanded compile-only/integration validation coverage (`api_surface_parity_test`, `decompiler_storage_hardening_test`), and updated lifter parity docs/blocker wording to reflect current non-scalar coverage boundary - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 21:45] Lifter parity follow-up - Added register-relative explicit helper-call argument-location hints (`MicrocodeValueLocationKind::RegisterRelative` via `consume_rrel`) and expanded validation coverage for malformed base-register hints; refreshed parity docs/blocker wording for the updated scalar-location envelope - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 22:10] Lifter parity follow-up - Added vector typed helper-call argument modeling (`MicrocodeValueKind::Vector`) with typed element width/count/sign/floating controls and explicit-location enforcement, expanded compile-only/integration validation coverage (`api_surface_parity_test`, `decompiler_storage_hardening_test`), and refreshed docs/blocker wording for current non-scalar coverage boundaries - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 22:35] Lifter parity follow-up - Added declaration-driven typed helper-call argument modeling (`MicrocodeValueKind::TypeDeclarationView`) with parse-time validation + explicit-location enforcement, expanded compile-only/integration validation coverage (`api_surface_parity_test`, `decompiler_storage_hardening_test`), and refreshed docs/blocker wording to include current type-declaration coverage - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 23:00] Lifter parity follow-up - Expanded helper-call option shaping with additional callinfo flags (`mark_dead_return_registers`, `mark_spoiled_lists_optimized`, `mark_synthetic_has_call`, `mark_has_format_string`) and kept compile-only/integration coverage current (`api_surface_parity_test`, `decompiler_storage_hardening_test`); refreshed parity docs language for broadened typed helper-call support - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 23:20] Lifter parity follow-up - Expanded helper-call option shaping with scalar callinfo field hints (`callee_address`, `solid_argument_count`, `call_stack_pointer_delta`, `stack_arguments_top`) mapped to `mcallinfo_t`, added validation coverage for negative solid-argument counts, and updated compile-only parity field checks for the new options - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 23:45] Lifter parity follow-up - Added plugin action-context host bridges (`ActionContext` host handles + `widget_host`/`with_widget_host`/`decompiler_view_host`/`with_decompiler_view_host`), wired runtime population in action adapters (including pseudocode-view `vdui` discovery), updated lifter/docs coverage, and expanded compile-only/integration validation coverage (`api_surface_parity_test`, `loader_processor_scenario_test`) - Evidence: `cmake --build build --target idax_api_surface_check idax_loader_processor_scenario_test idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|loader_processor_scenario|decompiler_storage_hardening"` (3/3 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 23:58] Appcall open-point hardening - Extended appcall-smoke launch attempts in `ida2py_port` to include `--wait` fixture mode, updated host fixture to support timed hold mode, refreshed runtime/docs evidence, and reran open-point sweep; appcall remains blocked with `start_process failed (return code: -1)` while full matrix + Lumina smoke pass - Evidence: `scripts/run_open_points.sh build-open-points-surge2 RelWithDebInfo` (full=pass, appcall=blocked, lumina=pass), `build-open-points-surge2/logs/appcall-smoke.log` shows failed `--wait` + default launch attempts.
- [2026-02-14 23:59] Lifter probe follow-up - Updated `lifter_port_plugin` snapshot flow to consume `with_decompiler_view_host` and report per-context host availability, keeping real-port pressure on new action-context host bridges; verified build + integration/compile-only regressions remain green - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check idax_loader_processor_scenario_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|loader_processor_scenario|decompiler_storage_hardening"` (3/3 pass).
- [2026-02-14 24:10] Appcall open-point hardening - Added external spawn+attach fallback probing to `ida2py_port --appcall-smoke`, tightened open-point classifier to treat `attach_process failed` as blocked-backend evidence, refreshed docs/evidence paths, and reran open-point sweep; appcall remains blocked (`start_process=-1`, `attach_process=-4`) while full matrix + Lumina smoke pass - Evidence: `scripts/run_open_points.sh build-open-points-surge4 RelWithDebInfo` (full=pass, appcall=blocked, lumina=pass), `build-open-points-surge4/logs/appcall-smoke.log`.
- [2026-02-14 24:25] Lifter parity follow-up - Expanded helper-call option shaping with declaration-based return-type hints (`return_type_declaration`), added validation coverage for malformed return declarations in decompiler hardening tests, and updated compile-only parity checks/docs; full regression remains green - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 24:40] Lifter parity planning hardening - Completed a source-backed gap matrix in `docs/port_gap_audit_lifter.md` by mapping concrete `<userhome>/dev/lifter` AVX/VMX microcode patterns to idax coverage status and ordering closure work into additive API slices (P0 instruction builder/emitter, P1 callinfo depth, P2 placement controls, P3 typed view ergonomics); synced Findings + Decision log entries to keep blocker execution auditable - Evidence: updated `docs/port_gap_audit_lifter.md`, `agents.md` Findings #136 and Decision Log entry.
- [2026-02-14 24:55] Lifter parity incremental closure - Added baseline generic typed microcode instruction emission in `ida::decompiler` (`MicrocodeOpcode`, `MicrocodeOperandKind`, `MicrocodeOperand`, `MicrocodeInstruction`, `MicrocodeContext::emit_instruction`, `MicrocodeContext::emit_instructions`), expanded integration + compile-only coverage (`decompiler_storage_hardening_test`, `api_surface_parity_test`), and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `docs/namespace_topology.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 25:10] Lifter parity incremental closure - Added constrained placement-policy controls for typed microcode emission (`MicrocodeInsertPolicy`, `emit_instruction_with_policy`, `emit_instructions_with_policy`), mapped policies to SDK insertion semantics (`insert_into_block` after-anchor, `nullptr` for beginning), expanded integration + compile-only coverage (`decompiler_storage_hardening_test`, `api_surface_parity_test`), and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `docs/namespace_topology.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 25:25] Lifter parity incremental closure - Expanded helper-call callinfo shaping with semantic role + return-location hints (`MicrocodeFunctionRole`, `MicrocodeCallOptions::{function_role, return_location}`), mapped role hints to SDK `funcrole_t`, added return-location validation/mapping for helper-call options, expanded integration + compile-only coverage (`decompiler_storage_hardening_test`, `api_surface_parity_test`), and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `docs/namespace_topology.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 25:40] Lifter parity incremental closure - Extended helper-call placement controls with `MicrocodeCallOptions::insert_policy` (sharing `MicrocodeInsertPolicy` semantics), updated helper-call insertion routing in `src/decompiler.cpp`, expanded compile-only/integration coverage (`api_surface_parity_test`, `decompiler_storage_hardening_test`), and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 25:42] Decompiler hardening stabilization - During integration expansion, direct successful helper-call emissions with aggressive role/return-location options in the filter path produced `INTERR: 50765` in subsequent decompile steps on this fixture; adjusted tests to keep these options covered via validation-focused paths while retaining full suite stability - Evidence: `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 25:55] Lifter parity incremental closure - Expanded helper-call register-return typing to support declaration-driven return types with explicit size matching and wider-register UDT marking in `emit_helper_call_with_arguments_to_register_and_options`, added validation coverage for return-type size mismatch paths (`decompiler_storage_hardening_test`), and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 26:10] Lifter parity incremental closure - Expanded helper-call register-argument typing so `MicrocodeValueKind::Register` accepts declaration-driven types (`type_declaration`) with parse-time validation and size-match enforcement; kept integer-width fallback when no declaration is provided, expanded integration coverage for invalid declaration/size/register-id paths (`decompiler_storage_hardening_test`), and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 26:25] Lifter parity incremental closure - Expanded typed helper-call arguments with optional metadata (`argument_name`, `argument_flags`, `MicrocodeArgumentFlag`) mapped to `mcallarg_t` name/flags with unsupported-bit validation and `FAI_RETPTR` hidden-flag normalization; expanded compile-only/integration coverage (`api_surface_parity_test`, `decompiler_storage_hardening_test`) and refreshed parity docs (`docs/namespace_topology.md`, `docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 26:40] Lifter probe closure increment - Added VMX microcode-lifting subset to `examples/plugin/lifter_port_plugin.cpp` using public idax microcode-filter/context APIs (`register_microcode_filter`, typed helper-call emission + typed arg/call options), updated port-audit/matrix/examples docs to reflect the executable subset, and revalidated plugin build + full test suite - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 26:55] Lifter parity incremental closure - Added temporary-register allocation to `ida::decompiler::MicrocodeContext` (`allocate_temporary_register`), expanded compile-only/integration validation coverage (`api_surface_parity_test`, `decompiler_storage_hardening_test`), refreshed decompiler parity docs (`docs/api_reference.md`, `docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`), and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 27:10] Lifter probe closure increment - Extended `examples/plugin/lifter_port_plugin.cpp` VMX subset with AVX `vzeroupper` no-op lowering, updated examples/port-audit/matrix docs to reflect mixed VMX/AVX executable coverage, and revalidated plugin build + full test suite - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 27:25] Lifter parity incremental closure - Added helper-call default callinfo shaping by inferring `solid_argument_count` from emitted argument lists when not explicitly set, expanded compile-only/integration validation coverage (`api_surface_parity_test`, `decompiler_storage_hardening_test`), refreshed decompiler parity docs (`docs/api_reference.md`, `docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`), and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 27:40] Lifter parity incremental closure - Added helper-call auto-stack placement shaping controls (`auto_stack_start_offset`, `auto_stack_alignment`) with validation in `build_call_arguments`, expanded compile-only/integration validation coverage for malformed option paths (`api_surface_parity_test`, `decompiler_storage_hardening_test`), refreshed decompiler parity docs (`docs/api_reference.md`, `docs/port_gap_audit_lifter.md`), and revalidated target tests - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass).
- [2026-02-14 27:55] Lifter probe closure increment - Extended `examples/plugin/lifter_port_plugin.cpp` with AVX scalar math/conversion lowering (`vaddss/vsubss/vmulss/vdivss`, `vaddsd/vsubsd/vmulsd/vdivsd`, `vcvtss2sd`, `vcvtsd2ss`) via typed microcode emission while keeping VMX subset lifting active; refreshed lifter docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`, `docs/sdk_domain_coverage_matrix.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 28:00] Lifter probe hardening - Documented operand-width inference nuance (decoded operand value objects do not expose operand text) and constrained AVX scalar lowering to deterministic XMM-width copy semantics in the probe; synced Findings + Decision log + blocker wording/docs for the updated subset scope - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass), docs updated (`docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `examples/README.md`).
- [2026-02-14 28:10] Lifter probe closure increment - Expanded AVX scalar subset in `examples/plugin/lifter_port_plugin.cpp` with scalar min/max/sqrt/move lowering (`vminss/vmaxss/vminsd/vmaxsd`, `vsqrtss/vsqrtsd`, `vmovss/vmovsd`) using typed microcode emission + helper-return composition; refreshed lifter docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`, `docs/sdk_domain_coverage_matrix.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 28:20] Lifter probe hardening - Reordered scalar-move lowering to handle memory-destination `vmovss`/`vmovsd` paths before destination-register loading, avoiding unnecessary load failures for store-only forms; revalidated plugin build + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 28:30] Lifter probe closure increment - Expanded AVX coverage in `examples/plugin/lifter_port_plugin.cpp` with baseline packed math/move lowering (`vaddps/vsubps/vmulps/vdivps`, `vaddpd/vsubpd/vmulpd/vdivpd`, `vmov*` packed families) using typed microcode emission + operand-text width heuristics + store-aware move handling; refreshed lifter docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`, `docs/sdk_domain_coverage_matrix.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 28:40] Lifter parity incremental closure - Expanded helper-call register-return fallback typing in `src/decompiler.cpp` so widths beyond scalar integer sizes synthesize byte-array return types instead of `Unsupported`, enabling packed helper-return flows without explicit return declarations; refreshed docs (`docs/api_reference.md`) and revalidated full suite stability - Evidence: `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 28:50] Lifter probe closure increment - Expanded packed AVX subset in `examples/plugin/lifter_port_plugin.cpp` with packed min/max/sqrt lowering (`vminps/vmaxps/vminpd/vmaxpd`, `vsqrtps/vsqrtpd`) using helper-return composition over widened destination types; refreshed docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 29:00] Lifter probe closure increment - Expanded packed AVX subset in `examples/plugin/lifter_port_plugin.cpp` with baseline conversion lowering (`vcvtps2pd`/`vcvtpd2ps`, `vcvtdq2ps`/`vcvtudq2ps`, `vcvtdq2pd`/`vcvtudq2pd`) via typed microcode emission and operand-width heuristics; refreshed docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 29:10] Lifter probe closure increment - Expanded packed AVX subset in `examples/plugin/lifter_port_plugin.cpp` with helper-fallback conversion families (`vcvt*2dq/udq/qq/uqq`, including truncating variants) using typed helper-call return emission + destination-width inference; refreshed docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 29:20] Lifter probe closure increment - Expanded packed AVX subset in `examples/plugin/lifter_port_plugin.cpp` with addsub/horizontal helper-call lowering (`vaddsub*`, `vhadd*`, `vhsub*`), refreshed docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`, `docs/api_reference.md`, `docs/sdk_domain_coverage_matrix.md`), and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 29:30] Lifter probe closure increment - Expanded packed AVX subset in `examples/plugin/lifter_port_plugin.cpp` with variadic helper-fallback bitwise/permute/blend families (mixed register/immediate operand forwarding), refreshed docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`, `docs/sdk_domain_coverage_matrix.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 29:40] Lifter probe closure increment - Expanded packed AVX subset in `examples/plugin/lifter_port_plugin.cpp` with variadic helper-fallback shift/rotate families (`vps*`, `vprol*`, `vpror*`), refreshed docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`, `docs/sdk_domain_coverage_matrix.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 29:50] Lifter probe closure increment - Expanded packed AVX subset in `examples/plugin/lifter_port_plugin.cpp` with tolerant variadic helper-fallback compare/misc coverage (`vcmp*`/`vpcmp*`, `vdpps`, `vround*`/`vrcp*`/`vrsqrt*`/`vget*`/`vfixup*`/`vscale*`/`vrange*`/`vreduce*`, `vbroadcast*`/`vextract*`/`vinsert*`/`vunpck*`/`vmov*dup`/`vmaskmov*`), degrading unsupported operand shapes to `NotHandled`; refreshed docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`, `docs/sdk_domain_coverage_matrix.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 30:00] Lifter probe hardening - Extended variadic helper fallback to handle memory-source operands via effective-address pointer arguments and to no-op unsupported compare mask destinations, while widening misc fallback families (`gather/scatter/compress/expand`, `popcnt/lzcnt/gfni/pclmul/aes/sha`, `movnt/movmsk/pmov/pinsert/extractps/insertps`, `pack/phsub/fmaddsub`) for broader practical parity; refreshed docs (`docs/port_gap_audit_lifter.md`, `examples/README.md`, `docs/sdk_domain_coverage_matrix.md`) and revalidated plugin + full test suite stability - Evidence: `cmake --build build --target idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-14 30:10] Lifter parity incremental closure - Added typed packed bitwise/shift opcode support in `ida::decompiler` (`MicrocodeOpcode` + SDK mapping in `src/decompiler.cpp`) and updated `lifter_port_plugin` to emit typed bitwise/shift instructions for direct register/immediate forms before helper fallback; expanded compile-only parity checks and revalidated plugin + full suite stability - Evidence: `cmake --build build --target idax_api_surface_check idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 00:20] Lifter parity incremental closure - Added `MicrocodeOpcode::Subtract` in `ida::decompiler` (`include/ida/decompiler.hpp`, `src/decompiler.cpp` mapping to `m_sub`) and expanded `examples/plugin/lifter_port_plugin.cpp` to route packed integer add/sub families (`vpadd*`/`vpsub*`) through typed emission before helper fallback; refreshed docs for the updated typed-opcode coverage and revalidated focused + full regression suites - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 00:45] Lifter parity hardening - Expanded packed integer routing in `examples/plugin/lifter_port_plugin.cpp` so `vpadd*`/`vpsub*` remain typed-first while helper fallback now also covers memory-source and saturating integer forms (`vpaddsb`/`vpaddsw`/`vpaddusb`/`vpaddusw`, `vpsubsb`/`vpsubsw`/`vpsubusb`/`vpsubusw`); revalidated plugin build plus focused and full regression suites - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 01:10] Lifter parity incremental closure - Added `MicrocodeOpcode::Multiply` in `ida::decompiler` (`include/ida/decompiler.hpp`, `src/decompiler.cpp` mapping to `m_mul`), expanded `examples/plugin/lifter_port_plugin.cpp` to route safe packed integer multiply forms (`vpmulld`/`vpmullq`) through typed emission before helper fallback, and added non-direct multiply fallback coverage (`vpmullw`/`vpmuludq`/`vpmaddwd`) while keeping integer add/sub dual-path routing; refreshed docs (`docs/api_reference.md`, `docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`, `examples/README.md`) and revalidated focused + full regression suites - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 01:35] Lifter parity hardening - Expanded probe-side typed packed binary emission in `examples/plugin/lifter_port_plugin.cpp` to support two-operand encodings by treating destination as implicit left source, preserving typed-first behavior for add/sub/mul/bitwise/shift families before helper fallback; revalidated plugin build plus focused and full regression suites - Evidence: `cmake --build build --target idax_lifter_port_plugin idax_api_surface_check` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 02:20] Lifter parity write-path expansion - Extended `ida::decompiler` with richer non-scalar typed writable IR construction/mutation semantics: new typed operand/value mop kinds (`RegisterPair`/`GlobalAddress`/`StackVariable`/`HelperReference`), declaration-driven vector element typing for helper-call arguments, and advanced callinfo list shaping in `MicrocodeCallOptions` (`return_registers`, `spoiled_registers`, `passthrough_registers`, `dead_registers`, `visible_memory_ranges`, `visible_memory_all`) with validation (including pass-reg subset rules). Expanded compile-only parity coverage and decompiler hardening validation checks, refreshed docs (`docs/api_reference.md`, `docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`), and revalidated focused + full regression suites - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test idax_lifter_port_plugin` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 03:10] Lifter parity typed-operand increment - Added `MicrocodeOperandKind::BlockReference` with validated `MicrocodeOperand::block_index` mapping to SDK `mop_t::make_blkref`, expanded compile-only and decompiler hardening validation coverage (including negative-index validation path), refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`), and revalidated full build+test suite - Evidence: `cmake --build build` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 03:35] Lifter parity typed-operand increment - Added `MicrocodeOperandKind::NestedInstruction` with shared typed payload support (`MicrocodeOperand::nested_instruction`) and recursive/depth-limited validation mapped through SDK `mop_t::create_from_insn`; expanded compile-only and decompiler hardening validation coverage (including missing-nested-payload validation), refreshed parity docs (`docs/api_reference.md`, `docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`), and revalidated focused + full test suites - Evidence: `cmake --build build` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 04:05] Lifter parity typed-operand increment - Added `MicrocodeOperandKind::LocalVariable` with `MicrocodeOperand::{local_variable_index, local_variable_offset}` mapped to SDK local-variable mop construction (`_make_lvar`) plus validation paths; expanded compile-only and decompiler hardening validation coverage (including invalid local-variable-index path), refreshed parity docs (`docs/api_reference.md`, `docs/port_gap_audit_lifter.md`, `docs/sdk_domain_coverage_matrix.md`), and revalidated focused + full test suites - Evidence: `cmake --build build` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 04:35] Lifter parity local-variable value-path increment - Added `MicrocodeValueKind::LocalVariable` with index/offset fields and guarded context query helper `MicrocodeContext::local_variable_count()`, expanded lifter probe rewrite path usage (`vzeroupper` now prefers local-variable self-move when locals exist with no-op fallback), and added focused hardening coverage for local-variable value validation + context query in `decompiler_storage_hardening_test`; also cleaned unintended fixture dirt (`tests/fixtures/simple_appcall_linux64.i64`) - Evidence: `cmake --build build` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass), `git status --short` confirms `.i64` cleanup.
- [2026-02-15 04:55] Lifter probe local-variable rewrite expansion - Consolidated local-variable self-move emission into shared helper (`try_emit_local_variable_self_move`) and applied it to a second VMX rewrite site (`vmxoff`) in addition to `vzeroupper`; expanded decompiler hardening coverage with a second rewrite-attempt check and revalidated build + full suite stability while keeping fixture clean - Evidence: `cmake --build build` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass), `git status --short` shows only source/test/docs tracker updates.
- [2026-02-15 15:57] Appcall backend hardening - Added debugger backend APIs (`BackendInfo`, `available_backends`, `current_backend`, `load_backend`) and queued launch/attach wrappers (`request_start`, `request_attach`), then upgraded `ida2py_port --appcall-smoke` to auto-load appcall-capable backends and try `start`/`request_start` + `attach`/`request_attach` with explicit state checks; expanded compile-only + integration coverage for new debugger surface - Evidence: `cmake --build build --target idax_api_surface_check idax_debugger_ui_graph_event_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|debugger_ui_graph_event"` (2/2 pass), `cmake --build build-port-gap --target idax_ida2py_port` (pass).
- [2026-02-15 15:57] Open-point evidence refresh - Re-ran open-point closure sweep after backend/request hardening; full matrix and Lumina smoke still pass while Appcall remains host-blocked with richer diagnostics (backend loaded, `start_process` rc `0` + request-start no-process, `attach_process` rc `-1` + request-attach no-process); updated compatibility/validation/appcall/ida2py docs and API index/topology wording - Evidence: `scripts/run_open_points.sh build-open-points-surge5 RelWithDebInfo` (full=pass, appcall=blocked, lumina=pass), `build-open-points-surge5/logs/appcall-smoke.log`, docs updated (`docs/compatibility_matrix.md`, `docs/validation_report.md`, `docs/appcall_runtime_validation.md`, `docs/port_gap_audit_ida2py.md`, `docs/api_reference.md`, `docs/namespace_topology.md`).
- [2026-02-15 16:30] Appcall smoke stabilization - Added bounded queue-drain settling helper in `examples/tools/ida2py_port.cpp` for request fallbacks (`request_start`/`request_attach` now run multi-cycle `run_requests` + short delays before classifying `NoProcess`); revalidated parity/integration and rebuilt tool target - Evidence: `cmake --build build --target idax_api_surface_check idax_debugger_ui_graph_event_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|debugger_ui_graph_event"` (2/2 pass), `cmake --build build-port-gap --target idax_ida2py_port` (pass).
- [2026-02-15 16:35] Appcall host evidence refresh - Re-ran fixture-backed appcall smoke after request-settle hardening; host remains blocked with stronger classification (`start_process` rc `0` + request-settle still `NoProcess`, `attach_process` rc `-1` + request-settle still `NoProcess`) - Evidence: `scripts/build_appcall_fixture.sh build-port-gap/fixtures/simple_appcall_host` (pass), `build-port-gap/examples/idax_ida2py_port --quiet --appcall-smoke build-port-gap/fixtures/simple_appcall_host` (blocked with enriched diagnostics).
- [2026-02-15 16:50] Open-point sweep refresh - Re-ran full open-point closure automation after request-settle hardening and refreshed compatibility/validation evidence paths to `build-open-points-surge6`; full matrix still passes, Lumina smoke still passes, and Appcall remains host-blocked with bounded-settle diagnostics - Evidence: `scripts/run_open_points.sh build-open-points-surge6 RelWithDebInfo` (full=pass, appcall=blocked, lumina=pass), docs updated (`docs/compatibility_matrix.md`, `docs/validation_report.md`, `docs/appcall_runtime_validation.md`).
- [2026-02-15 17:20] Lifter placement parity follow-up - Added policy-aware low-level microcode emit APIs in `ida::decompiler::MicrocodeContext` (`emit_noop_with_policy`, `emit_move_register_with_policy`, `emit_load_memory_register_with_policy`, `emit_store_memory_register_with_policy`), routed existing low-level emit helpers through policy defaults, and updated compile-only/integration coverage plus lifter parity docs - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), docs updated (`docs/port_gap_audit_lifter.md`).
- [2026-02-15 17:55] Lifter low-level UDT semantics increment - Added additive `mark_user_defined_type` overloads for decompiler low-level move/load/store emit APIs (with and without placement-policy variants), wired UDT flag mapping in `src/decompiler.cpp`, expanded compile-only + decompiler hardening coverage for the new overloads, and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 18:20] Lifter writeback UDT semantics increment - Added additive `store_operand_register(..., mark_user_defined_type)` overload in `ida::decompiler::MicrocodeContext`, wired source-mop UDT flag mapping in `src/decompiler.cpp`, expanded compile-only + decompiler hardening coverage, and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_api_surface_check idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "api_surface_parity|decompiler_storage_hardening"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 18:45] Lifter immediate typed-argument semantics increment - Expanded helper-call argument building so `UnsignedImmediate`/`SignedImmediate` accept optional declaration-driven typing (`type_declaration`) with parse/size validation and declaration-width inference, added decompiler hardening coverage for malformed/mismatched immediate declarations, and refreshed parity docs (`docs/port_gap_audit_lifter.md`, `docs/api_reference.md`) - Evidence: `cmake --build build --target idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "decompiler_storage_hardening|api_surface_parity"` (2/2 pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 19:05] Lifter callinfo validation semantics increment - Tightened `passthrough_registers` subset validation in helper-call option shaping (`src/decompiler.cpp`) so passthrough ranges are always required to be included in `spoiled_registers`, expanded decompiler hardening coverage for pass-without-spoil rejection, and refreshed parity docs (`docs/port_gap_audit_lifter.md`) - Evidence: `cmake --build build --target idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "decompiler_storage_hardening"` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
- [2026-02-15 19:30] Lifter callinfo coherence hardening - Ensured helper-call callinfo invariants by always merging `return_registers` into `spoiled` in option application (`src/decompiler.cpp`), so passthrough-subset validation remains coherent with callinfo semantics; revalidated decompiler hardening and full suite stability - Evidence: `cmake --build build --target idax_decompiler_storage_hardening_test` (pass), `ctest --test-dir build --output-on-failure -R "decompiler_storage_hardening"` (pass), `ctest --test-dir build --output-on-failure` (16/16 pass).
