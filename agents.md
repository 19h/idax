...
- Severity: High
- Description: The current public wrapper still lacks decompiler write-path depth required by lifter (rich writable microcode IR emission/mutation and raw view-handle context for advanced per-view manipulations).
...
- Long-term mitigation: Add additive decompiler write-path APIs (richer structured microcode emit/edit facade) plus carefully-scoped view-handle context bridges while preserving public opacity.
...
6. Prioritize additive decompiler write-path design for lifter-class ports: richer writable microcode IR helpers (beyond baseline filter hooks) and carefully-scoped raw view-handle context bridging for advanced per-view manipulation flows.
