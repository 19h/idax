...
- Severity: High
- Description: The current public wrapper still lacks decompiler write-path depth required by lifter (non-integer typed writable microcode IR construction/mutation including vector/UDT helper-call argument + advanced callinfo/tmop modeling and typed mop builders, plus raw view-handle context for advanced per-view manipulations).
...
- Long-term mitigation: Add additive decompiler write-path APIs (richer typed microcode value/argument/callinfo construction facade beyond current integer helper + lightweight call-option support) plus carefully-scoped view-handle context bridges while preserving public opacity.
...
6. Prioritize additive decompiler write-path design for lifter-class ports: typed microcode IR value/argument/call builders beyond current integer-helper + lightweight call-option coverage (vector/UDT argument + advanced callinfo/tmop modeling) and carefully-scoped raw view-handle context bridging for advanced per-view manipulation flows.
