...
- Long-term mitigation: Add additive decompiler write-path APIs (richer typed microcode value/argument/callinfo construction facade beyond current integer/float/byte-array/vector/type-declaration helper + lightweight call-option/location-hint support) plus carefully-scoped view-handle context bridges while preserving public opacity.
...
6. Prioritize additive decompiler write-path design for lifter-class ports: typed microcode IR value/argument/call builders beyond current integer/float/byte-array/vector/type-declaration + lightweight call-option/location-hint coverage (advanced vector/UDT semantics + advanced callinfo/tmop semantics) and carefully-scoped raw view-handle context bridging for advanced per-view manipulation flows.
