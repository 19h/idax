...
145. [2026-02-14] lifter probe closure increment: `examples/plugin/lifter_port_plugin.cpp` now installs a working VMX lifting subset through public idax microcode-filter APIs, including helper-call lowering for `vmxon/vmxoff/vmcall/vmlaunch/vmresume/vmptrld/vmptrst/vmclear/vmread/vmwrite/invept/invvpid/vmfunc`. Impact: the port probe now exercises real instruction-to-helper lowering paths instead of shell-only action wiring. Mitigation: keep expanding toward AVX-heavy handlers as deeper writable IR surfaces are added.
