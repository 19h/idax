...
| [`data`] | Byte-level I/O | Read (`read_byte` .. `read_qword`, `read_bytes`, `read_string`, `read_typed`), write, patch, originals, fixed-width and processor-aware definitions, owned custom type/format registration with retained panic-safe callbacks, attachment/list/query APIs, explicit/inferred custom creation, byte-length string/structure definitions and `undefine`, `find_binary_pattern` |
