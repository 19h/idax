...
| [`data`] | Byte-level I/O | Read (`read_byte` .. `read_qword`, `read_bytes`, `read_string`, `read_typed`), write, patch, originals, fixed-width element definitions (`define_byte` .. `define_zword`, `define_tbyte`, `define_float`, `define_double`), byte-length string/structure definitions and `undefine`, `find_binary_pattern` |
